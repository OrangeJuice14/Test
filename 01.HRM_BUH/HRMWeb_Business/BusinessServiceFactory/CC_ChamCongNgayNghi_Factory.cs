...
namespace HRMWeb_Business.BusinessServiceFactory
{
    public class CC_ChamCongNgayNghi_Factory : BaseFactory<Entities, CC_ChamCongNgayNghi>
    {
        ...
        public IQueryable<DTO_ChamCongNgayNghi_Find> DangKyChamCongNgayNghi_Find(int thang, int nam, Guid idNhanVien)
        {
            ...
            var result = (from o in this.ObjectSet
                          where o.TuNgay <= cuoiThang && o.DenNgay >= dauThang
        ...
        public IQueryable<DTO_ChamCongNgayNghi_Find> FindForChamCongNgayNghi(int thang, int nam, Guid? boPhanId, string maNhanSu, Guid webUserId, Guid? idLoaiNhanSu)
        {
            ...
            var result = (from o in this.ObjectSet
                          ...
                          where (
                                ...
                                // lấy danh sách nhân viên lúc đăng ký thuộc bộ phận mình quản lý 
                                // hoặc nhân viên được chuyển đến bộ phận mình quản lý
                                // hoặc tất cả nhân viên trong đơn vị con (bao gồm cả trưởng đơn vị con) // 13/02/2020
                                && (danhSachPhongBanPhanQuyen.Any(x => x.Oid == o.IDBoPhan) 
                                || danhSachPhongBanPhanQuyen.Any(x => x.Oid == o.ThongTinNhanVien.NhanVien.BoPhan)
                                || danhSachPhongBanPhanQuyen.Any(x => x.Oid == o.ThongTinNhanVien.NhanVien.BoPhan1.BoPhanCha))
