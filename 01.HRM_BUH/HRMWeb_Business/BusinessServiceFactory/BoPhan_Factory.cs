...
namespace HRMWeb_Business.BusinessServiceFactory
{
    public class BoPhan_Factory : BaseFactory<Entities, BoPhan>
    {
        ...
        {//su dung cho cham cong only
            // 26/08/2019 lấy tất cả bộ phận để phân quyền
        ...
        {//su dung cho cham cong only
            Guid bpc = new Guid(WebConfigurationManager.AppSettings["BoPhanCha"]);
            var webGroupId = WebUser_Factory.New().GetByID(webUserId)?.WebGroupID;
            var result = (from o in this.Context.BoPhans
                          where (o.WebUser_BoPhan.Any(x => x.IDWebUser == webUserId) || webGroupId == new Guid("00000000-0000-0000-0000-000000000001"))
                          && o.NgungHoatDong == false
