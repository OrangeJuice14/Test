...
namespace HRMWebApp.KPI.Core.Controllers
{
    public class PlanKPIApiController : ApiController
    {
        ...
        //            {
        //                //so sánh mã ngạch
        //                List<AgentObject_Salary> agents_SalaryList = session.Query<AgentObject_Salary>().Where(sa => sa.ScaleSalary != null ? sa.ScaleSalary.Id == staff.StaffSalaryInfo.ScaleSalary.Id : false).ToList();
                        
        //                if (agents_SalaryList.Count > 1) 
        //                {
        //                    if (staff.StaffLevel.Qualification != null) //so sánh trình độ chuyên môn
        //                        agents_SalaryList = agents_SalaryList.Where(sa => sa.Qualification != null ? sa.Qualification.Id == staff.StaffLevel.Qualification.Id : false).ToList();
        //                }
        //                if (agents_SalaryList.Count > 1) 
        //                {
        //                    if (staff.StaffLevel.AcademicTitle != null) //so sánh học hàm
        //                        agents_SalaryList = agents_SalaryList.Where(sa => sa.AcademicTitle != null ? sa.AcademicTitle.Id == staff.StaffLevel.AcademicTitle.Id : false).ToList();
        //                }

        //                if (agents_SalaryList.Count > 0)
        //                {
        //                    List<AgentObject> agentObjects = session.Query<AgentObject>().Where(sa => sa.Id == agents_SalaryList.First().AgentObject.Id).ToList();
        //                    agentObjects = agentObjects.Where(sa => sa.AgentObjectType.PlanTypes.Any(pt => pt.Id == planType)).ToList();
        //                    foreach (AgentObject ob in agentObjects)
        //                    {
        //                        UserPlanKPIDTO planDTO = plan.Map<UserPlanKPIDTO>();
        //                        if (planDTO != null)
