...
using HRMWebApp.KPI.Core.DTO.ApiResultDTO;
    ...
    public class DepartmentApiController : ApiController
    {
        ...
        public IEnumerable<DepartmentDTO> GetList()
        {
            ...
            SessionManager.DoWork(session =>
            {
                ...
                result = session.Query<Department>().Where(d => d.ParentDepartment.Id == buh && d.GCRecord == null && d.IsDisable == false).OrderBy(d => d.Name).ToList().Map<DepartmentDTO>();
            });
            return result;
        }
        [Authorize]
        ...
        [Authorize]
        [Route("")]
        public IEnumerable<CanBoPhongBanDTO> GetListCanBoPhongban(string MaPhongBan)
        {
            var result = new List<CanBoPhongBanDTO>();
            ...
            using (connection)
            {
                ...
                sqlCom.CommandText = "WebService_DanhSachNhanVienTheoPhongBan";
                sqlCom.CommandType = CommandType.StoredProcedure;
                sqlCom.Parameters.Add(new SqlParameter("@MaPhongBan", MaPhongBan));
                SqlDataReader reader = sqlCom.ExecuteReader();
                while (reader.Read())
                {
                    CanBoPhongBanDTO item = new CanBoPhongBanDTO();
                    item.MaCanBo = reader["MaQuanLy"].ToString();
                    item.HoTen = reader["HoTen"].ToString();
                    var date = Convert.ToDateTime(reader["NgaySinh"].ToString());
                    item.NgaySinh = date.ToString("MM/dd/yyyy");
                    item.HocHam = reader["HocHam"].ToString();
                    item.HocVi = reader["HocVi"].ToString();
                    item.HeSoLuong = reader["HeSoLuong"].ToString();
                    var ngayluong  = Convert.ToDateTime(reader["NgayHuongLuong"].ToString());
                    item.NgayLenLuong = ngayluong.ToString("MM/dd/yyyy");
                    result.Add(item);
                }
            }
            return result;
