...
namespace HRMWeb_Service
{
    ...
    public partial class Service1 : IService1
    {
        ...
        public String GetSoChungTuByKyTinhLuong(String publicKey, String token, Guid kyTinhLuong)
        {
            ...
            {
                KyTinhLuong_Factory factory = KyTinhLuong_Factory.New();
                var list = Utils.DataClassHelper.spd_Service_GetSoChungTuHienWeb(kyTinhLuong);
                return JsonConvert.SerializeObject(list);
            }
            else return null;
        }
        public String TongHopTienGiangVaThuNhapKhac(String publicKey, String token, Guid nhanVien, Guid kyTinhLuong, Guid soChungTu)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                var list = Utils.DataClassHelper.spd_Service_TongHopTienGiangVaThuNhapKhac(kyTinhLuong, soChungTu, nhanVien);
                return JsonConvert.SerializeObject(list);
            }
            else return null;
        }
        public String TongHopThueTNCNTrongThang(String publicKey, String token, Guid nhanVien, Guid kyTinhLuong, Guid soChungTu)
        {
            ...
            {
                KyTinhLuong_Factory factory = KyTinhLuong_Factory.New();
                var list = Utils.DataClassHelper.spd_Service_TongHopThueTNCNTrongThang(nhanVien, kyTinhLuong, soChungTu).FirstOrDefault();
                return JsonConvert.SerializeObject(list);
            }
            else return null;
        }
