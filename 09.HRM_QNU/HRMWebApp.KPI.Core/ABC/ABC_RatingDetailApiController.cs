...
namespace HRMWebApp.KPI.Core.Controllers
{
    public class ABC_RatingDetailApiController : ApiController
    {
                        ...
                        if (rating == null)
                        {
                            ...
                            else if (staffType == 2 && result.EvaluationBoardType == 2)
                            {
                                foreach (ABC_RatingGroupDTO group in result.ABC_RatingGroupDTOs)
                                {
                                    ...
                                    foreach (ABC_CriterionDetail criDetail in criterionDetails)
                                    {
                                        ABC_RatingDetail rDetail = ParseNewRatingDetailSixMonth(group, criDetail, rating, rating.Staff.Id, rating.Department.Id, evaluationId, session);
                                        session.Save(rDetail);
                            ...
                            else if (result.EvaluationBoardType == 1)
                            {
                                foreach (ABC_RatingGroupDTO group in result.ABC_RatingGroupDTOs)
                                {
                                    ...
                                    foreach (ABC_CriterionDetail criDetail in criterionDetails)
                                    {
                                        ABC_RatingDetail rDetail = ParseNewRatingDetailYear(group, criDetail, rating, rating.Staff.Id, rating.Department.Id, evaluationId, session);
                                        session.Save(rDetail);
                        ...
                        else
                        {
                            ...
                            foreach (ABC_RatingGroupDTO group in result.ABC_RatingGroupDTOs)
                            {
                                ...
                                foreach (ABC_RatingDetail ratingDetail in ratingDetails)
                                {
                                    ...
                                    {
                                        ParseRatingDetail_Supervisor(ratingDetail, rating.Staff.Id, rating.Department.Id, evaluationId, session);
                                        session.SaveOrUpdate(ratingDetail);
                                    }
        ...
        public ABC_RatingDetail ParseNewRatingDetailSixMonth(ABC_RatingGroupDTO group, ABC_CriterionDetail criDetail, ABC_Rating rating, Guid staffId, Guid departmentId, Guid evaluationId, NHibernate.ISession session)
        {
            ABC_RatingDetail rDetail = new ABC_RatingDetail();
            rDetail.Id = Guid.NewGuid();
            rDetail.ABC_Criterion = new ABC_Criterion() { Id = group.Id };
        ...
        public ABC_RatingDetail ParseNewRatingDetailYear(ABC_RatingGroupDTO group, ABC_CriterionDetail criDetail, ABC_Rating rating, Guid staffId, Guid departmentId, Guid evaluationId, NHibernate.ISession session)
        {
            ABC_RatingDetail rDetail = new ABC_RatingDetail();
            rDetail.Id = Guid.NewGuid();
            rDetail.ABC_Criterion = new ABC_Criterion() { Id = group.Id };
        ...
        public static void ParseRatingDetail_Supervisor(ABC_RatingDetail ratingDetail, Guid staffId, Guid departmentId, Guid evaluationId, NHibernate.ISession session)
        {
            //Lấy tất cả tiêu chí cùng loại trong 6 tháng
            var listSamePlanDetail = session.Query<ABC_RatingDetail>().Where(r =>
            //Cùng loại tiêu chí
        ...
        public ABC_RatingDTO PutTempStaff(ABC_RatingDTO obj, NHibernate.ISession session)
        {
            ...
            foreach (ABC_RatingGroupDTO group in obj.ABC_RatingGroupDTOs)
            {
                foreach (ABC_RatingDetailDTO rd in group.ABC_RatingDetailDTOs)
                {
                    ...
                    if (obj.EvaluationBoardType == 3 || (obj.EvaluationBoardType == 2 && obj.StaffType == 1))
        ...
        public string GetCheckChildAllRatingLocked(ABC_Rating ratingResult, int staffType, NHibernate.ISession session)
        {
            ...
            if (eb.EvaluationBoardType == 1 && staffType == 1) //cả năm giảng viên
            {
                var listRating = session.Query<ABC_Rating>().Where(r =>
                    ...
                    && r.Department.Id == ratingResult.Department.Id
                    );
