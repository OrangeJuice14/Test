                        ...
                        if (obj != null)
                        {
                            ...
                            if (objFromDB != null)
                            {
                                ...
                                if (objFromDB.TrangThai == 1)
                                {
                                    ...
                                    factory.Context.spd_WebChamCong_CapNhatKhaiBaoCongTac(objFromDB.IDNhanVien, objFromDB.TuNgay, objFromDB.DenNgay, true);
                                }
                                else if (objFromDB.TrangThai == 0)
                                {
                                    //Cập nhật hình thức nghỉ trong chấm công theo ngày là: Không xác định
                                    factory.Context.spd_WebChamCong_CapNhatKhaiBaoCongTac(objFromDB.IDNhanVien, objFromDB.TuNgay, objFromDB.DenNgay, false);
                                }
                            }
                        }
                    }  
