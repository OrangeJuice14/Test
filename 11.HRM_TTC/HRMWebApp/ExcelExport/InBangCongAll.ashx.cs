    ...
    public class InBangCongAll : IHttpHandler
    {
        ...
        public void BangChamCongExportToExcelProcess(int thang, int nam, Guid userId, Guid groupId, Guid congTy, HttpContext context)
        {
            try
            {
                ...
                List<BoPhan> donViList = (new BoPhan_Factory()).GetDanhSachBoPhan_DuocPhanQuyenChoWebUserId_GCRecordIsNull_New(userId, congTy).Where(q => q.CongTy == congTy).ToList();
                ...
                SetCellFormat(sheet, rowIndex + 1, 2, 4 + songay, true, false, false, false, true);
                rowIndex++;
                rowIndex++;

                var result = factory.QuanLyChamCong_ThongTinChamCongThangAll(thang, nam, Guid.Empty, string.Empty, Guid.Empty, userId, congTy).ToList();
                ...
                foreach (var donVi in donViList)
                {
                    ...
                    var dataList = result.Where(q => q.IDBoPhan == donVi.Oid);

                    //
                    index = 1;
                    foreach (DTO_QuanLyChamCong_ThongTinChamCongThang ccThang in dataList)
                    {
