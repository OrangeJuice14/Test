...
namespace HRMWebApp.ExcelExport
{
    public class InBangCong : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            Guid webGroupId = context.Request.Params["webGroupId"] != null ? new Guid(context.Request.Params["webGroupId"].ToString()) : Guid.Empty;
            ...
            if (currentUser != null)
            {
                BangChamCongExportToExcelProcess(thang, nam, bophanId, idNhanVien, webGroupId, userId, null, Guid.Empty, currentUser.CongTyId.Value, context);
            }
        }

        public void BangChamCongExportToExcelProcess(int thang, int nam, Guid bophanId, Guid idNhanVien, Guid webGroupId, Guid userId, string maNhanSu, Guid idLoaiNhanSu, Guid congTy, HttpContext context)
        {
            try
            {
                ...
                if (webGroupId.Equals(idCaNhan))
                    result = factory.QuanLyChamCong_ThongTinChamCongThang_Cua1NhanVien(thang, nam, idNhanVien, congTy);
                else
                    result = factory.QuanLyChamCong_ThongTinChamCongThang(thang, nam, bophanId, maNhanSu, idLoaiNhanSu, userId, congTy);
                //
                BoPhan DonVi = (new BoPhan_Factory()).GetByID(bophanId);

                //
                List<DTO_NgayChamCong> listNgay = factory.GetList_NgayTrongKyChamCong(thang, nam, bophanId, webGroupId, congTy).ToList();
                ...
                merged1.Value = DonVi.CongTy1.BoPhan.TenBoPhan.ToUpper();
                ...
                //Tên đơn vị
                WorksheetMergedCellsRegion merged6 = sheet.MergedCellsRegions.Add(rowIndex, 0, rowIndex, 3);
                merged6.Value = "ĐƠN VỊ: " + DonVi.TenBoPhan.ToUpper();
                ...
                merged6.CellFormat.Font.Bold = ExcelDefaultableBoolean.True;
                ...
                #region TargetGroup
                //Ngày trong tháng
                BackgroundColor(sheet, rowIndex, 0, songay + 10, Color.LightGray);
                BackgroundColor(sheet, rowIndex + 1, 2, songay + 10, Color.LightGray);
                BackgroundColor(sheet, rowIndex + 2, 0, songay + 10, Color.LightGray);
                ...
                WorksheetMergedCellsRegion mergedSoNgayCong = sheet.MergedCellsRegions.Add(rowIndex, songay + 4, rowIndex, songay + 10);
                ...
                WorksheetMergedCellsRegion mergedNgayHe = sheet.MergedCellsRegions.Add(rowIndex + 1, songay + 7, rowIndex + 2, songay + 7);
                mergedNgayHe.Value = "Ngày hè";
                SetRegionBorder(mergedNgayHe, true, false, true, true);
                mergedNgayHe.CellFormat.Alignment = HorizontalCellAlignment.Center;

                WorksheetMergedCellsRegion mergedNgayPhepBu = sheet.MergedCellsRegions.Add(rowIndex + 1, songay + 8, rowIndex + 2, songay + 8);
                mergedNgayPhepBu.Value = "Ngày phép bù";
                SetRegionBorder(mergedNgayPhepBu, true, false, true, true);
                mergedNgayPhepBu.CellFormat.Alignment = HorizontalCellAlignment.Center;

                WorksheetMergedCellsRegion mergedKhongLuong = sheet.MergedCellsRegions.Add(rowIndex + 1, songay + 9, rowIndex + 2, songay + 9);
                ...
                WorksheetMergedCellsRegion mergedTongCong = sheet.MergedCellsRegions.Add(rowIndex + 1, songay + 10, rowIndex + 2, songay + 10);
                ...
                foreach (DTO_QuanLyChamCong_ThongTinChamCongThang ccThang in result)
                {
                    ...
                    sheet.Rows[rowIndex].Cells[idx + 3].Value = ccThang.NgayHe;
                    sheet.Rows[rowIndex].Cells[idx].CellFormat.Alignment = HorizontalCellAlignment.Center;
                    CellBorder(sheet, rowIndex, idx + 3);

                    sheet.Rows[rowIndex].Cells[idx + 4].Value = ccThang.NgayPhepBu;
                    sheet.Rows[rowIndex].Cells[idx].CellFormat.Alignment = HorizontalCellAlignment.Center;
                    CellBorder(sheet, rowIndex, idx + 4);

                    sheet.Rows[rowIndex].Cells[idx + 5].Value = ccThang.NgayKhongLuong;
                    sheet.Rows[rowIndex].Cells[idx].CellFormat.Alignment = HorizontalCellAlignment.Center;
                    CellBorder(sheet, rowIndex, idx + 5);

                    sheet.Rows[rowIndex].Cells[idx + 6].Value = ccThang.TongCong;
                    sheet.Rows[rowIndex].Cells[idx].CellFormat.Alignment = HorizontalCellAlignment.Center;
                    CellBorder(sheet, rowIndex, idx + 6);
                    //
                    rowIndex++;
                    index++;
                }
            ...
            {
                Helpers.Helper.ErrorLog("InBangCong.ashx/BangChamCongExportToExcelProcess", ex);
                throw ex;
            }
        }
