...
namespace HRMWeb_Service
{
    ...
    public partial class Service1 : IService1
    {
        ...
        public bool CaNhanKhaiBaoCongTac_KiemTraTuNgayDenNgayCoHopLe(String publicKey, String token, Guid? khaiBaoCongTacOid, DateTime tuNgay, DateTime denNgay, Guid webUserId, string IDNhanVien, int buoi)
        {//DANG SD
            tuNgay = tuNgay.Date;
            ...
            if (Helper.TrustTest(publicKey, token))
            {
                if (string.IsNullOrEmpty(IDNhanVien))
                {
                    ...
                    if (trungHoacGiaoNgay_BoSungCong)
                        hopLe = trungHoacGiaoNgay_BoSungCong;
                    //
                    return hopLe;
                }
                else
        ...
        public bool CaNhanKhaiBaoCongTac_KhaiBaoMoi(String publicKey, String token, String noiDung, String diaDiem, DateTime tuNgay, DateTime denNgay, byte buoi, int gioBatDau, int phutBatDau, int gioKetThuc, int phutKetThuc, Guid webUserId, Guid nguoiKy, string IDNhanVien, Guid congTy)
        {
            try
            {
                if (Helper.TrustTest(publicKey, token))
                {
                    ...
                    if (string.IsNullOrEmpty(IDNhanVien)) // Đăng ký công tác
                    {
                        //
                        CC_KhaiBaoCongTac_Factory factory = CC_KhaiBaoCongTac_Factory.New();
                        ...
                        if (webUser.WebGroupID == WebGroupConst.TruongPhongID)
                        {
                        ...
                        {
                            newObj.SoNgay = HamDungChung.GetBusinessDays(tuNgay, denNgay); // Nghỉ thứ 7
                        }
                        else
                        {
                        ...
                        {
                            //Cập nhật hình thức nghỉ trong chấm công theo ngày là: Làm cả ngày
                            factory.Context.spd_WebChamCong_CapNhatKhaiBaoCongTac(newObj.IDNhanVien, newObj.TuNgay, newObj.DenNgay, true, buoi);
                        }
                        ...
                        if (idCaNhan.Equals(webUser.WebGroupID))
                        {
                            ...
                            if (cauHinh != null)
                            {
                                KhaiBaoCongTac_SetDataSendMail(newObj, cauHinh.EmailSender, cauHinh.PassSender, webUser.EmailTP, webUser.Oid);
                            }
                        }
                        //
                        return true;
                    }
                    else // Quản lý công tác
                    {
                        using (var transaction = new TransactionScope(TransactionScopeOption.Required, TimeSpan.FromSeconds(360)))
                        {
                            ...
                            if (webUser.WebGroupID == WebGroupConst.TruongPhongID || webUser.WebGroupID == WebGroupConst.TruongPhongUyQuyenID)
                            {
        ...
        public void KhaiBaoCongTac_SetDataSendMail(CC_KhaiBaoCongTac newObj, string emailSender, string passSender, string emailReceiver, Guid user)
        {
            string tieudeguimail = "DUYỆT KHAI BÁO CÔNG TÁC";
            string noidungguimail = "Họ tên: [" + newObj.HoSo.HoTen.ToUpper() + "] Đơn vị: [" + newObj.HoSo.NhanVien.BoPhan1.TenBoPhan.ToUpper() + "] xin nghỉ: [Đi Công Tác] Từ ngày [" + String.Format("{0:dd/MM/yyyy}", newObj.TuNgay) + "] Đến ngày [" + String.Format("{0:dd/MM/yyyy}", newObj.DenNgay) + "]";
            //Gửi mail
