    ...
    public partial class Service1 : IService1
    {
        ...
        public IEnumerable<DTO_ChiTietChamCongNhanVien> GetList_ChiTietChamCongNhanVien(String publicKey, String token)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                ...
                IEnumerable<DTO_ChiTietChamCongNhanVien> list = factory.GetAll().Map<DTO_ChiTietChamCongNhanVien>();
                return list;
            }
            else
            {
        ...
        public bool Delete_ChiTietChamCongNhanVienBy_Id(String publicKey, String token, Guid id)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                ...
                if (obj != null)
                {
                    try
                    {
                        CC_ChiTietChamCongNhanVien_Factory.FullDelete(factory.Context, obj);
                        factory.SaveChanges();
                        return true;
                    }
                    catch (Exception ex)
        ...
        public bool DeleteList_ChiTietChamCongNhanVienBy_IdList(String publicKey, String token, List<Guid> idList)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                ...
                if (objList != null)
                {
                    try
                    {
                        CC_ChiTietChamCongNhanVien_Factory.FullDelete(factory.Context, objList);
                        factory.SaveChanges();
                        return true;
                    }
                    catch (Exception ex)
