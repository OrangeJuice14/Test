...
namespace HRMWeb_Service
{
    ...
    {
        public IEnumerable<DTO_HinhThucNghi> ChamCongNgayNghi_GetList_HinhThucNghi(String publicKey, String token, Guid congTy)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                CC_HinhThucNghi_Factory factory = CC_HinhThucNghi_Factory.New();
                IEnumerable<DTO_HinhThucNghi> list = factory.ChamCongNgayNghi_GetHinhThucNghiList(congTy).Map<DTO_HinhThucNghi>();
                return list;
            }
            else
            {
        ...
        public List<String> GetList_HinhThucNghiKyHieu(String publicKey, String token)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                ...
                CC_HinhThucNghi_Factory factory = CC_HinhThucNghi_Factory.New();
                IEnumerable<DTO_HinhThucNghi> temp = factory.GetKyHieu_All().Map<DTO_HinhThucNghi>();
        ...
        public String ChamCongNgayNghi_GetList_HinhThucNghi_Json(String publicKey, String token, Guid congTy)
        {//DANG SD
            IEnumerable<DTO_HinhThucNghi> list = ChamCongNgayNghi_GetList_HinhThucNghi(publicKey, token,congTy);
            String json = JsonConvert.SerializeObject(list);
            return json;
        }
