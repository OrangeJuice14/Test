...
namespace HRMWeb_Service
{
    ...
    public partial class Service1 : IService1
    {
                                    ...
                                    foreach (var item in listWebGroup_BoPhan)
                                    {
                                        if (item.BoPhanIds.Contains(objFromDB.IDBoPhan.Value))
                                        {
                                                ...
                                                || idQuanTriKhoi.Equals(item.WebGroupId.ToString().ToUpper()))
                                            {
                                                ...
                                                ChamCongNgayNghi_GuiMailChoNguoiDangKy(objFromDB, cauHinh.EmailSender, cauHinh.PassSender, objFromDB.ThongTinNhanVien.NhanVien.HoSo.Email, user.Oid);
                                                ...
                                                if (!objFromDB.IsBanGiamHieu.Value)
                                                {
                                                    ...
                                                    if (objFromDB.SoNgay > soNgayHieuTruongDuyet)
                                                    {
                                                        ...
                                                        objFromDB.PhanHoi_TP = obj.PhanHoi_TP;
                                                        objFromDB.PhanHoi_HT = obj.PhanHoi_HT;
                                                    }
                                                }
                                                if (objFromDB.IsBanGiamHieu.Value)
                                                {
                                                    ...
                                                    objFromDB.PhanHoi_HT = obj.PhanHoi_HT;
                                                    objFromDB.PhanHoi_HDQT = obj.PhanHoi_HDQT;
                                                }
                                            }
                                            //Hội đồng quản trị
                                                     ...
                                                     || idHieuTruongUyQuyen.Equals(item.WebGroupId.ToString().ToUpper()))
                                            {
                                                ...
                                                if (user != null && objFromDB.IsBanGiamHieu.Value)
                                                {
                                                    ...
                                                    ChamCongNgayNghi_SetDataSendMail(objFromDB, cauHinh.EmailSender, cauHinh.PassSender, user.EmailHDQT, user.Oid);
                                                }
                                            }
                                            //Trưởng phòng
                                                     ...
                                                     || idTruongPhongUyQuyen.Equals(item.WebGroupId.ToString().ToUpper()))
                                            {
                                                ...
                                                if (user != null && !objFromDB.IsBanGiamHieu.Value)
                                                {
                                                    ...
                                                    {
                                                        //Gửi đến hiệu trưởng
                                                        ChamCongNgayNghi_SetDataSendMail(objFromDB, cauHinh.EmailSender, cauHinh.PassSender, user.EmailHT, user.Oid);
                                                    }
                ...
                if (Helper.TrustTest(publicKey, token))
                {
                        ...
                        TimeSpan.FromSeconds(360)))
                    {
                        ...
                        if (hoSo != null)
                        {
                            ...
                            {
                                newObj.SoNgay = HamDungChung.GetBusinessDays(tuNgay, denNgay); // Nghỉ t7
                            }
                            else
                            {
                                newObj.SoNgay = HamDungChung.GetBusinessDays1(tuNgay, denNgay); // Làm thứ 7
                            }
                                ...
                                || userHienTai.WebGroupID.Equals(idHieuTruongUQ))
                            {
                                ...
                                {
                                    //Gửi đến hội đồng quản trị
                                    ChamCongNgayNghi_SetDataSendMail(newObj, cauHinh.EmailSender, cauHinh.PassSender, userHienTai.EmailHDQT, userHienTai.Oid);
                                }
                            }
                                ...
                                || userHienTai.WebGroupID.Equals(idTruongPhongUyQuyen))
                            {
                                ...
                                if (!newObj.IsBanGiamHieu.Value)
                                {
                                    ...
                                    {
                                        //Gửi đến hiệu trưởng
                                        ChamCongNgayNghi_SetDataSendMail(newObj, cauHinh.EmailSender, cauHinh.PassSender, userHienTai.EmailHT, userHienTai.Oid);
                                    }
                                    if (newObj.IsTruongPhong.Value)
                                    {
                                        //Gửi đến hiệu trưởng
                            ...
                            if (userHienTai.WebGroupID.Equals(idCaNhan))
                            {
                                ...
                                else
                                {
                                    ...
                                    ChamCongNgayNghi_SetDataSendMail(newObj, cauHinh.EmailSender, cauHinh.PassSender, userHienTai.EmailTP, userHienTai.Oid);
                                }
                            }
        ...
        public void ChamCongNgayNghi_SetDataSendMail(CC_ChamCongNgayNghi newObj, string emailSender, string passSender, string emailReceiver, Guid user)
        {
            string tieudeguimail = "DUYỆT " + newObj.CC_HinhThucNghi.TenHinhThucNghi.ToUpper();
            string noidungguimail = "Họ tên: [" + newObj.ThongTinNhanVien.NhanVien.HoSo.HoTen.ToUpper() + "] Đơn vị: [" + newObj.BoPhan.TenBoPhan.ToUpper() + "] xin nghỉ: [" + newObj.CC_HinhThucNghi.TenHinhThucNghi + "] Từ ngày [" + String.Format("{0:dd/MM/yyyy}", newObj.TuNgay) + "] Đến ngày [" + String.Format("{0:dd/MM/yyyy}", newObj.DenNgay) + "]";
            //Gửi mail
        ...
        /// </summary>
        public void ChamCongNgayNghi_GuiMailChoNguoiDangKy(CC_ChamCongNgayNghi newObj, string emailSender, string passSender, string emailReceiver, Guid user)
        {
            string tieudeguimail = "ĐƠN XIN " + newObj.CC_HinhThucNghi.TenHinhThucNghi.ToUpper() + " ĐÃ ĐƯỢC DUYỆT";
            string noidungguimail = "Họ tên: [" + newObj.ThongTinNhanVien.NhanVien.HoSo.HoTen.ToUpper() + "] Đơn vị: [" + newObj.BoPhan.TenBoPhan.ToUpper() + "] xin nghỉ: [" + newObj.CC_HinhThucNghi.TenHinhThucNghi + "] Từ ngày [" + String.Format("{0:dd/MM/yyyy}", newObj.TuNgay) + "] Đến ngày [" + String.Format("{0:dd/MM/yyyy}", newObj.DenNgay) + "]";
            //Gửi mail
            if (!string.IsNullOrEmpty(emailSender) && !string.IsNullOrEmpty(passSender))
                QuanLyGuiEmail_SendMail(emailSender, passSender, emailReceiver, tieudeguimail, noidungguimail, user);
        ...
        public bool DangKyNghiPhep_TaoMoi(String publicKey, String token, Guid nhanVienID, String noiDung, String noiNghiPhep, String tenDonXinNghi, String diaChiLienHe, int loaiDonXinNghi, DateTime tuNgay, DateTime denNgay, Guid webUserId)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                    ...
                    TimeSpan.FromSeconds(360)))
                {
                    ...
                    newObj.TrangThai_HDQT = -1;
                    newObj.TrangThai_TP = -1;
                    newObj.SoNgay = HamDungChung.GetBusinessDays(tuNgay, denNgay);
                    newObj.TenGiayXinPhep = tenDonXinNghi;
                    try
                    {
                        factory.SaveChangesWithoutTransactionScope();
        ...
        {//DANG SD
            ...
            if (Helper.TrustTest(publicKey, token))
            {
                ...
                var trungHoacGiaoNgay_BoSungCong = (from o in factory.Context.CC_BoSungCong
                                                    where o.IDNhanVien == nhanVienID
                                                            && (tuNgay <= o.DenNgay && denNgay >= o.TuNgay)
                                                            && (o.Buoi == buoi || buoi == 1)
                                                    select true).FirstOrDefault();
        ...
        public void QuanLyGuiEmail_SendMail(string emailgui, string passgui, string emailnhan, string tieude, string noidung, Guid user)
        {
            //
            try
            {   //
                ...
                {
                    //
                    sucess = GuiMail(tieude, noidung, emailgui, passgui, emailnhan);
                }
                //
                if (sucess)
                {
        ...
        private bool GuiMail(string tieude, string noidung, string emailgui, string passgui, string emailnhan)
        {
            bool sucess = false;
            try
            {
                if (System.Configuration.ConfigurationManager.AppSettings["SendMailToStaffMail"] == "true") {
            ...
            {//
                Helper.ErrorLog("Service1_ChucNangChamCongNgayNghi/GuiMail", ex);
                sucess = false;
            }
            //
            return sucess;
