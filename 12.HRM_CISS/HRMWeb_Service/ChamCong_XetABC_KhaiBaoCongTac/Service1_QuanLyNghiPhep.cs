...
namespace HRMWeb_Service
{
    ...
    public partial class Service1 : IService1
    {
        ...
        public bool QuanLyNghiPhepNam_SaveList(String publicKey, String token, Guid Oid, string TongSoNgayPhep, string SoNgayPhepCongThem, string SoNgayPhepNamTruoc, string SoNgayTamUngHienTai, string SoNgayBuNamHienTai)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                CC_QuanLyNghiPhep_Factory factory = CC_QuanLyNghiPhep_Factory.New();
                ...
                if (objFromDB != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(SoNgayBuNamHienTai))
                        objFromDB.SoNgayBuNamHienTai = Convert.ToDecimal(SoNgayBuNamHienTai);
                    //
            ...
            if (Helper.TrustTest(publicKey, token))
            {
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, TimeSpan.FromSeconds(180)))
                {
                    try
                    {
                        ...
                        factory.SaveChanges();
                        //
                        factory.Context.spd_WebChamCong_TaoDanhSachPhepDauNam(quanLy.CongTy, quanLy.Oid, nienDoTaiChinh, false);
                        //
        ...
        public bool QuanLyNghiPhepNam_Remove(String publicKey, String token, Guid nienDoTaiChinh, Guid congTy)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, TimeSpan.FromSeconds(180)))
                {
                    try
                    {
                        ...
                        var factory = CC_QuanLyNghiPhep_Factory.New();
                        //
                        factory.Context.spd_WebChamCong_TaoDanhSachPhepDauNam(congTy, Guid.Empty, nienDoTaiChinh, true);
                        //
                        transaction.Complete();
                        return true;
                    }
