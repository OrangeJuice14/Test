define(['app/app', 'app/services/kpi/methodService', 'app/services/kpi/planKPIDetailService', 'app/services/kpi/departmentService', 'app/services/kpi/staffService', 'app/services/kpi/criterionService', 'app/services/kpi/professorCriterionService', 'app/services/kpi/planKPIDetail_KPIService', 'app/services/kpi/professorOtherActivityService', 'app/services/kpi/targetGroupDetailService', 'app/services/kpi/fileAttachmentService','app/services/kpi/scienceResearchService', 'moment'], function (app) {
            ...
            $scope.methodList = [];
                ...
                $scope.viceDepartmentStaffResource = {
                    ...
                    dataSource: {
                        transport: {
                            read: function (options) {
                                return staffService.getViceDepartmentStaff($scope.agentObjectTypeId, $scope.agentObjectTypeId == 4 ? MANAGER.GUID_EMPTY : department.departmentId).then(function (result) {
                                    options.success(result.data);
                                });
                            }
                        }
            ...
            $scope.treeOptions = {
                ...
                $scope.deleteMethod = function (Id) {
                    ...
                    methodService.getObj(Id).then(function (result) {
                        ...
                        methodService.DeleteDetail(result.data).then(function () {

                            if (result.data == 1) {
                                alert("Xóa thất bại!");
                            }
                            $scope.detailGrid.dataSource.read();
                            $scope.detailGrid.refresh();
                        });
                    });                                   
                }
 ...
 alert("Lưu thành công");
    ...
    app.controller('methodDetailController', ['$scope', '$modalInstance', 'id', 'planKPIDetailId', 'planStaffId', 'targetGroupDetailId', 'methodService', 'planKPIDetailService',
            function ($scope, $modalInstance, id, planKPIDetailId, planStaffId, targetGroupDetailId, methodService, planKPIDetailService) {
                $scope.isNew = id == MANAGER.GUID_EMPTY || '' ? true : false;
