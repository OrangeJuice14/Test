...
define(['app/app', 'app/services/kpi/planKPIService', 'app/services/kpi/agentObjectService', 'app/services/kpi/staffService','app/services/kpi/departmentService'], function (app) {
    ...
    app.controller('planKPIController', ['$scope', '$modal', '$rootScope', 'planKPIService', 'agentObjectService','staffService',
            function ($scope, $modal, $rootScope, planKPIService, agentObjectService, staffService) {
                ...
                $scope.mainGridOptions = {
                    ...
                    selectable: true,
                    columns: [{
                        template: "<a ng-click='SchollManagePlan(\"#:data.Id #\")' href='javascript:void(0)'>#:data.Name #</a>",
                        width: "60%",
                    },
                    {
                        template: "<div><a ng-click='Edit(\"#:data.Id #\")' class='btn btn-labeled btn-success'><i class='btn-label fa fa-pencil'></i>Chỉnh sửa</a></div>",
                        width: "15%"
                    },
           ...
           $scope.DepartmentManagePlan = function (Id) {
               ...
               if (Id == '406f6e70-c3d9-49d8-b6f6-e39649ba8458')
               {
                       staffService.getCurrentStaff().then(function (result2) {
                           ...
                           $modalInstance.close();
                           window.location = "/#/schoolManagePlankpidetail/" + $scope.planId + "/4731710F-94FB-4EF5-9F48-05E12648AA62/" + $scope.normalStaffId + "/1";
                       });                  
               }
               else
               {
                   ...
                   staffService.getDepartmentLeaderId(Id).then(function (result2) {
                       ...
                       $modalInstance.close();
                       window.location = "/#/plankpidetail/" + $scope.planId + "/" + $scope.agentObjectId + "/" + $scope.normalStaffId + "/1/0";
                   });
               });
               }              
           }
           $scope.FacultyManagePlan = function (Id) {
               staffService.getDepartmentLeaderAgentObjectId(Id).then(function (result) {
                   ...
                   staffService.getDepartmentLeaderId(Id).then(function (result2) {
                       $scope.normalStaffId = result2.data;
                       $modalInstance.close();
                       window.location = "/#/facultyManagePlankpidetail/" + $scope.planId + "/" + $scope.agentObjectId + "/" + $scope.normalStaffId + "/1/0";
                   });
               });
           }
