...
namespace HRMWeb_Service
{
    public partial interface IService1
    {
        ...
        IEnumerable<DTO_ChamCongNgayNghi_Find> ChamCongNgayNghi_Find(String publicKey, String token, int thang,
            int nam, Guid? boPhanId, String maNhanSu, Guid webUserId, Guid? idLoaiNhanSu);

        [OperationContract]
        String ChamCongNgayNghi_Find_Json(String publicKey, String token, int thang, int nam, Guid? boPhanId,
            String maNhanSu, Guid webUserId, Guid? idLoaiNhanSu);

        // Create //////////////////
        [OperationContract]
        DTO_CC_ChamCongNgayNghi ChamCongNgayNghi_TaoMoi(String publicKey, String token, Guid nhanVienID, String noiDung,
            Guid idHinhThucNghi, DateTime tuNgay, DateTime denNgay, Guid webUserId, Guid tinhThanh,int soNgay,int ngayDiDuong);
