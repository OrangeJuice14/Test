        ...
        private static void GetSetChild_WithBoPhanList(WebUser webUser)
        {
            ...
            //lay bo phan ma user duoc xu ly cham cong
            {
                if (webUser.HoSo != null)
                {
                    ...
                    try
                    {
                        webUser.TenBoPhan = webUser.HoSo.NhanVien.Department1.TenBoPhan;
                    }
                    catch (Exception)
                    {
                    }
                ...
                foreach (var webUser_BoPhan in webUser.WebUser_Department)
                {
                    //tmpList.Add(webUser_BoPhan.BoPhan.Map<DTO_BoPhan>());
                    DTO_BoPhan boPhanCanCheck = tmpList.SingleOrDefault(x => x.Oid == webUser_BoPhan.DepartmentID);
                    if (boPhanCanCheck != null)
                        boPhanCanCheck.Chon = true;
        ...
        public bool Save_WebUser(String publicKey, String token, DTO_WebUser obj)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                if (obj != null)
                {
                    ...
                    {
                        //cap nhat
                        objFromDB.CopyIncludedPropertiesFrom(obj, "UserName", "Password", "HoatDong", "WebGroupID");//.CopyPropertiesFrom(obj);
                        ...
                        if (obj.DanhSachDTO_BoPhan != null && obj.DanhSachDTO_BoPhan.ToList().Count > 0)
                            foreach (var dtoBoPhan in obj.DanhSachDTO_BoPhan)
                            {
                                if (dtoBoPhan.Chon)
                                {
                                    ...
                                    if (objForSave.WebUser_Department.All(x => x.DepartmentID != dtoBoPhan.Oid))
                                    {
                                        var wubp = new WebUser_Department();
                                        wubp.DepartmentID = dtoBoPhan.Oid;
                                        //wubp.IDWebUser = objForSave.Oid;
                                        objForSave.WebUser_Department.Add(wubp);

                                    }
                                }
                                else
                                {
                                    WebUser_Department objWebUser_BoPhanTimDuoc = objForSave.WebUser_Department.SingleOrDefault(x => x.DepartmentID == dtoBoPhan.Oid);
