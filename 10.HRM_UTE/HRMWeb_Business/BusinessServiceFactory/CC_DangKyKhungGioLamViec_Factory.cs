...
namespace HRMWeb_Business.BusinessServiceFactory
{
    public class CC_DangKyKhungGioLamViec_Factory : BaseFactory<Entities, CC_DangKyKhungGioLamViec>
    {
        ...
        public List<DTO_DangKyKhungGioLamViec> GetListDaDangKy(Guid? bophan, Guid ky, bool trangthaidoica)
        {
            ...
            result = (from o in this.ObjectSet
                      where o.ThongTinNhanVien1.NhanVien.BoPhan == bophan
                            && o.KyDangKy == ky
                            && ((trangthaidoica == true && o.CaChamCongMoi != null && (o.ChoPhepDoi ?? false) == false)
                                || trangthaidoica == false)
                      select new DTO_DangKyKhungGioLamViec()
                      {
                          ...
                          DuLieuDaThayDoi = o.CaChamCongMoi != null ? "Đổi khung giờ (Từ " + o.CC_CaChamCong.TenCa + " sang " + o.CC_CaChamCong1.TenCa + ")" : "",
                          TrangThaiDoiCa = o.ChoPhepDoi != null && o.ChoPhepDoi == true && o.CaChamCongMoi != null ? "Đồng ý" : ""
                      }).ToList();
        ...
        public List<DTO_DangKyKhungGioLamViec> GetListDaDangKyTatCa(Guid ky,bool trangthaidoica)
        {
            List<DTO_DangKyKhungGioLamViec> result = new List<DTO_DangKyKhungGioLamViec>();
            result = (from o in this.ObjectSet
                      where o.KyDangKy == ky
        ...
        public DTO_DangKyKhungGioLamViec DangKyKhungGio_GetDuLieuDaDangKy(Guid idNhanVien, Guid ky)
        {
            var obj = (from x in this.ObjectSet
                          ...
                          select new DTO_DangKyKhungGioLamViec
                          {
                             HoTen = x.ThongTinNhanVien1.NhanVien.HoSo.HoTen ,
                             TenKy =  y.TenKy,
                             TuNgay = y.TuNgay,
                             DenNgay = y.DenNgay,
                             TenCa = x.CC_CaChamCong.TenCa,
                             CaChamCong = x.CC_CaChamCong.Oid,
                             CaChamCongMoi = x.CaChamCongMoi,
                             KyDangKy = x.KyDangKy,
                             TuNgayThayDoi = x.TuNgay,
                             DenNgayThayDoi = x.DenNgay,
                             TenCaMoi = x.CC_CaChamCong1.TenCa,
                             ChoPhepDoi = x.ChoPhepDoi != null ? x.ChoPhepDoi.Value : false
                          }).Take(1).FirstOrDefault();
            //
            if (obj != null)
            {
                obj.DuLieuDaDangKy = "Cán bộ [" + obj.HoTen + "] đã đăng ký " + obj.TenKy + "( Từ " + String.Format("{0:dd/MM/yyyy}", obj.TuNgay) + " đến " + String.Format("{0:dd/MM/yyyy}", obj.DenNgay) + ")" + " với khung giờ là: " + obj.TenCa;

                if (obj.CaChamCongMoi != null)
                {
                    if (obj.ChoPhepDoi)
                        obj.DuLieuDaThayDoi = "Cán bộ [" + obj.HoTen + "] đã thay đổi khung giờ mới (Từ " + String.Format("{0:dd/MM/yyyy}", obj.TuNgayThayDoi) + " đến " + String.Format("{0:dd/MM/yyyy}", obj.DenNgayThayDoi) + ")" + " với khung giờ là: " + obj.TenCaMoi;
                    else
                        obj.DuLieuDaThayDoi = "Vui lòng đợi quản trị duyệt.";
                }
            }
            //
            return obj;
        ...
        public List<DTO_DangKyKhungGioLamViec> DangKyChamCong_Find(Guid? bophan, Guid ky, int trangthai, bool trangthaidoica)
        {
            ...
            if (bophan != null && bophan != Guid.Empty)
            {
                if (trangthai == 1)
                {
                    result1 = GetListDaDangKy(bophan,ky, trangthaidoica);
                }
                else if (trangthai == 0)
                {
                    result1 = GetListDaDangKy(bophan,ky, trangthaidoica);
            ...
            else
            {
                if (trangthai == 1)
                {
                    result1 = GetListDaDangKyTatCa(ky,trangthaidoica);
                }
                else if (trangthai == 0)
                {
                    result1 = GetListDaDangKyTatCa(ky, trangthaidoica);
