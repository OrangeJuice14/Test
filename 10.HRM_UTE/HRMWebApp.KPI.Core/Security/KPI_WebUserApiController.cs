    ...
    public class KPI_WebUserApiController : ApiController
    {
        ...
        public IEnumerable<AgentObjectDTO> GetListbyId(Guid classId)
        {
            ...
            SessionManager.DoWork(session =>
            {
                ...
                {
                    List<AgentObject_TargetGroup> ob = session.Query<AgentObject_TargetGroup>().Where(r => r.TargetGroupDetailId.Id == classId).ToList();
                    ...
                    foreach(AgentObject_TargetGroup item in ob)
                    {
