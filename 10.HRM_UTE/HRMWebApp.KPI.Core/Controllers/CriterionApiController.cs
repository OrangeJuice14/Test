...
namespace HRMWebApp.KPI.Core.Controllers
{
    public class CriterionApiController : ApiController
    {
        ...
        [Authorize]
        [Route("")]
        public IEnumerable<CriterionDictionaryDTO> GetDensityDictionary()
        {
            List<CriterionDictionaryDTO> result = new List<CriterionDictionaryDTO>();
            //Danh mục tỷ trọng cho khối phòng ban
            Guid targetGroupDetailId = new Guid(WebConfigurationManager.AppSettings["ChatLuongCongViec"]);
            SessionManager.DoWork(session =>
            {
                List<CriterionDictionary> temp = session.Query<CriterionDictionary>().Where(c => c.TargetGroupDetail.Id==targetGroupDetailId).OrderByDescending(c => c.Record).ToList();
                foreach (CriterionDictionary cri in temp)
                {
                    CriterionDictionaryDTO crd = new CriterionDictionaryDTO();
                    crd.Id = cri.Id;
                    crd.Name = cri.Name + " (" + cri.Record.ToString() + " - " + cri.MaxRecord.ToString() + ")";
                    crd.Record = cri.Record;
                    crd.MaxRecord = cri.MaxRecord;
                    result.Add(crd);
                }
            });
            return result;
        ...
        [Authorize]
        [Route("")]
        public CriterionDictionary GetDensityDictionaryById(Guid Id)
        {
            CriterionDictionary result = new CriterionDictionary();
       
            SessionManager.DoWork(session =>
            {
                result = session.Query<CriterionDictionary>().Where(c => c.Id== Id).SingleOrDefault();
            });
            return result;
        }
        ...
        [Authorize]
        [Route("")]
        public IEnumerable<ManageCodeDTO> GetListManageCode(int staffType)
        {
            List<ManageCodeDTO> result = new List<ManageCodeDTO>();
            SessionManager.DoWork(session =>
            {
                List<ManageCode> tempList = session.Query<ManageCode>().Where(p => p.StaffType == staffType).ToList();
                foreach(ManageCode m in tempList)
                {
                    ManageCodeDTO mcd = new ManageCodeDTO();
                    mcd.Id = m.Id;
                    mcd.Name = m.Name;
                    mcd.StaffType = m.StaffType;
                    mcd.ProfessorCriterion = m.ProfessorCriterion != null ? m.ProfessorCriterion.Map<ProfessorCriterionDTO>() : null;
                    result.Add(mcd);
                }
            });
            return result;
