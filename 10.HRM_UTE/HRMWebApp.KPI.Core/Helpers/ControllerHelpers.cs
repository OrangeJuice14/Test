...
namespace HRMWebApp.KPI.Core.Helpers
{
    ...
    public static class ControllerHelpers
    {
        ...
        public static PlanKPIDetail GetOriginalParentPlanKPIDetail(Guid pldId, NHibernate.ISession session)
        {
            ...
            //pld = GetPlanDetailDic(session)[pldId];
            pld = session.Query<PlanKPIDetail>().FirstOrDefault(p => p.Id == pldId);
            if (pld != null && pld.ParentPlanKPIDetail != null)
            {
                while (pld.ParentPlanKPIDetail != null)
                {
        ...
        public static PlanKPIDetail GetOriginalParentPlanKPIDetail(PlanKPIDetail pld, NHibernate.ISession session, Dictionary<Guid, Guid> listd)
        {
            Guid currentId = pld.Id;
            Guid temId = Guid.Empty;
            if (listd.ContainsKey(pld.Id))
            {
                temId = listd[pld.Id];
                while (temId != Guid.Empty)
                {
                    currentId = temId;
                    temId = listd[currentId];
                }
            }

            PlanKPIDetail detailresult = currentId != pld.Id ? session.Query<PlanKPIDetail>().SingleOrDefault(a => a.Id == currentId) : pld;
            return detailresult;
        }
        ...
        public static List<PlanKPIDetail> GetAllChildPlanKPIDetail(Guid pldId, NHibernate.ISession session)
        {
            ...
            if (pld != null)
            {
                //result = session.Query<PlanKPIDetail>().Where(p => GetOriginalParentPlanKPIDetail(p.Id, session).Id == pldId).ToList();

                //chỉ lấy những kế hoạch được nhận sau đó để cải thiện hiệu suất
                List<PlanKPIDetail> planDetailList = session.Query<PlanKPIDetail>().Where(q => q.ParentPlanKPIDetail != null && q.CreateTime > pld.CreateTime).ToList();
                foreach (PlanKPIDetail pdetail in planDetailList)
                {
                    //if (GetOriginalParentPlanKPIDetail(pdetail, session).Id == pldId)
                    //    result.Add(pdetail);

                    PlanKPIDetail pd = pdetail;
                    while (pd.ParentPlanKPIDetail != null)
                    {
                        pd = pd.ParentPlanKPIDetail;
                        if (pd.Id == pldId)
                            result.Add(pdetail);
                    }
                }
        ...
        public static List<MethodDTO> GetOriginalMethods(Guid pldId)
        {
            ...
            SessionManager.DoWork(session =>
            {
                ...
                if (originalPld != null)
                {
                    ...
                    {
                        MethodDTO item = new MethodDTO();
                        item.Id = me.Id;
                        item.Name = me.Name;
                        item.OrderNumber = me.OrderNumber;
                        item.StartTime = me.StartTime;
                        item.EndTime = me.EndTime;
                        // item.LeadDepartment = me.LeadDepartment != null ? new DepartmentDTO() { Id = me.LeadDepartment.Id,Name = me.LeadDepartment.Name } : null;
                        item.LeadDepartment = new List<Method_DepartmentDTO>();
                        foreach (Method_Department de in me.LeadDepartment)
                        {
                            Method_DepartmentDTO d = new Method_DepartmentDTO();
                            d.Id = de.Id;
                            d.DepartmentId = new DepartmentDTO() { Id = de.DepartmentId.Id, Name = de.DepartmentId.Name };
                            d.DiemSo = de.DiemSo;
                            item.LeadDepartment.Add(d);
                        }
                        item.SubStaff = new List<StaffDTO>();
                        item.StaffName = new List<string>();
                        item.StaffId = new List<Guid>();
                        foreach (var s in me.SubStaffs)
                        {
                            StaffProfile st = session.Query<StaffProfile>().Where(r => r.Id == s.Id).FirstOrDefault();
                            item.SubStaff.Add(new StaffDTO() { Id = s.Id, Name = st.Name });
                            item.StaffId.Add(s.Id);
                            item.StaffName.Add(st.Name);
                        }
                        result.Add(item);
        ...
        //***Dòng này Minh thêm mới ngày 10/06/2016 dùng để lấy thông tin chung từ store
        public static List<MethodDTO> GetOriginalMethods2(PlanKPIDetail pldId, NHibernate.ISession session, Dictionary<Guid, Guid> listd)
        {
            List<MethodDTO> result = new List<MethodDTO>();
            //PlanKPIDetail originalPld = GetOriginalParentPlanKPIDetail(pldId, session, listd);

            //sửa ngày 15/10/2016 gọi hàm khác để tối ưu performance
            PlanKPIDetail originalPld = GetOriginalParentPlanKPIDetail(pldId, session);
        ...
        public static PlanKPIDetail ParsePlanKPIDetail(PlanKPIMakingDetailDTO originalPlan, PlanKPIDetail updatePlanKPIDetail, PlanStaff planStaff, TargetGroupPlanMakingDTO tg, bool isNew)
        {
            ...
            if (originalPlan.CriterionDictionary != null && originalPlan.CriterionDictionary.Id != Guid.Empty)
            {
                pdn.CriterionDictionary = new CriterionDictionary() { Id = originalPlan.CriterionDictionary.Id };
            }
            else pdn.CriterionDictionary = null;
            ...
            // pdn.FromCriterion = originalPlan.FromCriterionId != Guid.Empty ? new Criterion() { Id = Guid.NewGuid() } : null;
            ...
            pdn.DanhMucMTCL = new DanhMucMTCL() { Id = originalPlan.DanhMucMTCL.Id };
            pdn.CapMucTieu = originalPlan.DanhMucMTCL.CapDanhMuc;
            //pdn.StaffLeader = originalPlan.StaffLeader != null ? new Staff() { Id = originalPlan.StaffLeader.Id } : new Staff() { Id = planStaff.Staff.Id };
            if (originalPlan.StaffLeader != null)
            {
                pdn.StaffLeader = new Staff() { Id = originalPlan.StaffLeader.Id };
            }
            else if (planStaff.Staff != null)
            {
                pdn.StaffLeader = new Staff() { Id = planStaff.Staff.Id };
            }
            ...
            if (isNew)
            {
                ...
                {
                    pdn.SubStaffs.Add(new PlanDetailSubStaff() { Staff = new Staff() { Id = did }, PlanKPIDetail = new PlanKPIDetail() { Id = pdn.Id } });
                }
            ...
            else
            {
                ...
                //Xoa staff cu
                List<Staff> listSubStaff = new List<Staff>();
                foreach (PlanDetailSubStaff plf in pdn.SubStaffs)
                {
                    listSubStaff.Add(plf.Staff);
                }

                List<Guid> originalSubStaffIds = pdn.SubStaffs.Select(s => s.Staff.Id).ToList();

                foreach (Guid osdid in originalSubStaffIds)
                {
                    //Staff removestaff = listSubStaff.Where(l => l.Id == osdid).SingleOrDefault();
                    PlanDetailSubStaff planDetailSubStaff = pdn.SubStaffs.SingleOrDefault(a => a.Staff.Id == osdid);
                    pdn.SubStaffs.Remove(planDetailSubStaff);


                }
                foreach (PlanDetailSubStaff pls in pdn.SubStaffs)
                {
                    pdn.SubStaffs.Remove(pls);
                }
                //pdn.SubStaffs = new List<PlanDetailSubStaff>();
                //Add staff moi
                foreach (PlanDetailSubStaffDTO pls in originalPlan.PlanDetailSubStaffs)
                {
                    pdn.SubStaffs.Add(new PlanDetailSubStaff() { Staff = new Staff() { Id = pls.StaffId }, PlanKPIDetail = new PlanKPIDetail() { Id = pdn.Id }, Density = pls.Density });
                }
        ...
        public static PlanKPIDetail ParsePlanKPIDetail(PlanKPIMakingDetailDTO originalPlan, PlanKPIDetail updatePlanKPIDetail, PlanStaff planStaff, TargetGroupPlanMakingDTO tg, bool isNew, NHibernate.ISession session)
        {
            PlanKPIDetail pdn = new PlanKPIDetail();
            if (!isNew)
            {
            ...
            if (originalPlan.DanhMucMTCL != null)
            {
                DanhMucMTCL danhmuc = session.Query<DanhMucMTCL>().Where(r => r.Id == originalPlan.DanhMucMTCL.Id).FirstOrDefault();
                pdn.CapMucTieu = danhmuc.CapDanhMuc;
                pdn.DanhMucMTCL = new DanhMucMTCL() { Id = originalPlan.DanhMucMTCL.Id };
            }
            else
                pdn.CapMucTieu = 0;
            ...
            //   pdn.CapMucTieu = originalPlan.CapMucTieu;
            pdn.MeasureUnit = originalPlan.MeasureUnitDTO != null ? new MeasureUnit() { Id = originalPlan.MeasureUnitDTO.Id } : null;
            pdn.TargetGroupDetail = new TargetGroupDetail() { Id = tg.TargetGroupId };
            pdn.PlanStaff = planStaff;
            pdn.StaffLeader = originalPlan.StaffLeader != null ? new Staff() { Id = originalPlan.StaffLeader.Id } : new Staff() { Id = planStaff.Staff.Id };
            ...
            else
            {
                ...
                foreach (Guid osdid in originalSubStaffIds)
                {
                    ...
                    PlanDetailSubStaff planDetailSubStaff = pdn.SubStaffs.SingleOrDefault(a => a.Staff.Id == osdid);
                    pdn.SubStaffs.Remove(planDetailSubStaff);
                    session.Delete(planDetailSubStaff);
                }
                foreach (PlanDetailSubStaff pls in pdn.SubStaffs)
                {
                    pdn.SubStaffs.Remove(pls);
                ...
                foreach (PlanDetailSubStaffDTO pls in originalPlan.PlanDetailSubStaffs)
                {
                    Staff staff = session.Query<Staff>().SingleOrDefault(a => a.Id == pls.StaffId);
                    PlanKPIDetail plkDetail = session.Query<PlanKPIDetail>().SingleOrDefault(a => a.Id == pdn.Id);
                    session.Save(new PlanDetailSubStaff() { Staff = staff, PlanKPIDetail = plkDetail, Density = pls.Density, NumberOfHour = pls.NumberOfHour });
                    //pdn.SubStaffs.Add(new PlanDetailSubStaff() { Staff = staff, PlanKPIDetail = plkDetail, Density = pls.Density });
                }
                List<Guid> newStaffIds = originalPlan.SubStaffIds.Where(p => !originalPlan.PlanDetailSubStaffs.Any(pp => pp.StaffId == p)).ToList();
                foreach (Guid did in newStaffIds)
                {
                    //Staff staff = session.Query<Staff>().SingleOrDefault(a => a.Id == did);
                    //PlanKPIDetail plkDetail = session.Query<PlanKPIDetail>().SingleOrDefault(a => a.Id == pdn.Id);
                    session.Save(new PlanDetailSubStaff() { Staff = new Staff() { Id = did }, PlanKPIDetail = new PlanKPIDetail() { Id = pdn.Id } });
                    //pdn.SubStaffs.Add(new PlanDetailSubStaff() { Staff = staff, PlanKPIDetail = plkDetail });
                }
                if (originalPlan.ActivityIds != null)
                {
                    foreach (Guid did in originalPlan.ActivityIds)
                ...
                case 1:
                    {
                        ...
                        foreach (PlanDetailSubStaff subs in cri.FromPlanKPIDetail.SubStaffs)
                        {
                            pd.SubStaffIds.Add(subs.Staff.Id);
                            pd.SubStaffNames.Add(subs.Staff.StaffProfile.Name);
                        }
                        foreach (FileAttachment d in cri.FromPlanKPIDetail.FileAttachments)
                        {
                        ...
                        pd.ManageCode = cri.FromPlanKPIDetail.ManageCode != null ? new ManageCodeDTO() { Id = cri.FromPlanKPIDetail.ManageCode.Id } : null;
                        if (pld.ParentPlanKPIDetail != null)
                        {
                            pd.DanhMucMTCL = new DanhMucMTCLDTO() { Id = pld.ParentPlanKPIDetail.DanhMucMTCL.Id, MaDanhMuc = pld.ParentPlanKPIDetail.DanhMucMTCL.MaDanhMuc, TenDanhMuc = pld.ParentPlanKPIDetail.DanhMucMTCL.TenDanhMuc };
                            pd.CapMucTieu = pld.ParentPlanKPIDetail.DanhMucMTCL.CapDanhMuc;
                        }
                        else if (pld.DanhMucMTCL != null)
                        {
                            pd.DanhMucMTCL = new DanhMucMTCLDTO() { Id = pld.DanhMucMTCL.Id, MaDanhMuc = pld.DanhMucMTCL.MaDanhMuc, TenDanhMuc = pld.DanhMucMTCL.TenDanhMuc };
                            pd.CapMucTieu = pld.DanhMucMTCL.CapDanhMuc;
                        }

                        PlanKPIDetail oriParentPlanDetail = ControllerHelpers.GetOriginalParentPlanKPIDetail(cri.FromPlanKPIDetail, session);
                        ...
                        if (oriParentPlanDetail.Methods != null)
                        {
                            foreach (Method m in oriParentPlanDetail.Methods)
                            {
                                ...
                                method.PlanKPIDetailId = m.PlanKPIDetail.Id;
                                method.StartTimeString = m.StartTime.ToString("dd/MM/yyyy");
                                method.EndTimeString = m.EndTime.ToString("dd/MM/yyyy");
                                //  method.LeadDepartment = m.LeadDepartment != null ? new DepartmentDTO() { Id = m.LeadDepartment.Id,Name = m.LeadDepartment.Name } : null;
                                method.LeadDepartment = new List<Method_DepartmentDTO>();
                                foreach (Method_Department de in m.LeadDepartment)
                                {
                                    ...
                                    d.DepartmentId = new DepartmentDTO() { Id = de.DepartmentId.Id, Name = de.DepartmentId.Name };
                                    d.DiemSo = de.DiemSo;
                                    method.LeadDepartment.Add(d);
                                }
                                method.SubStaff = new List<StaffDTO>();
                                method.StaffName = new List<string>();
                                method.StaffId = new List<Guid>();
                                foreach (var s in m.SubStaffs)
                                {
                                    StaffProfile st = session.Query<StaffProfile>().Where(r => r.Id == s.Id).FirstOrDefault();
                                    method.SubStaff.Add(new StaffDTO() { Id = s.Id, Name = st.Name });
                                    method.StaffId.Add(s.Id);
                                    method.StaffName.Add(st.Name);
                                }
                        ...
                        /// KPIs
                        if (oriParentPlanDetail.Id != Guid.Empty && oriParentPlanDetail.PlanKPIDetail_KPIs != null)
                        {
                            foreach (PlanKPIDetail_KPI m in oriParentPlanDetail.PlanKPIDetail_KPIs)
                            {
                ...
                case 2:
                    {
                        ...
                        {
                            pd.StaffLeader.Name = pd.StaffLeader.StaffProfile != null ? pd.StaffLeader.StaffProfile.Name : null;
                        }
                        ...
                        pd.MaxRecord = pld.MaxRecord; //Kế thừa trọng số chỉ KH từ 1 cấp cha (hoặc lấy từ chính nó)
                        ...
                        if (pld.ParentPlanKPIDetail != null)
                        {
                            pd.DanhMucMTCL = pld.ParentPlanKPIDetail.DanhMucMTCL != null ? new DanhMucMTCLDTO() { Id = pld.ParentPlanKPIDetail.DanhMucMTCL.Id, TenDanhMuc = pld.ParentPlanKPIDetail.DanhMucMTCL.TenDanhMuc, MaDanhMuc = pld.ParentPlanKPIDetail.DanhMucMTCL.MaDanhMuc } : null;
                            pd.CapMucTieu = pld.ParentPlanKPIDetail.DanhMucMTCL != null ? pld.ParentPlanKPIDetail.DanhMucMTCL.CapDanhMuc : 0;
                        }
                        else if (pld.DanhMucMTCL != null)
                        {
                            pd.DanhMucMTCL = new DanhMucMTCLDTO() { Id = pld.DanhMucMTCL.Id, TenDanhMuc = pld.DanhMucMTCL.TenDanhMuc, MaDanhMuc = pld.DanhMucMTCL.MaDanhMuc };
                            pd.CapMucTieu = pld.DanhMucMTCL.CapDanhMuc;
                        }
                        foreach (PlanDetailSubStaff subs in pld.SubStaffs)
                        {
                            pd.SubStaffIds.Add(subs.Staff.Id);
                            pd.PlanDetailSubStaffs.Add(new PlanDetailSubStaffDTO() { PlanKPIDetailId = pd.Id, StaffId = subs.Staff.Id, Density = subs.Density, NumberOfHour = subs.NumberOfHour, StaffName = subs.Staff.StaffProfile.Name });
                            pd.SubStaffNames.Add(subs.Staff.StaffProfile.Name);
                        }
                        ...
                        //nếu KH có criterion (đã khóa) hoặc cấp dưới đã nhận KH thì không thể xóa
                        if (pld.Criterions.Count > 0 || NumberChildPlanKPIDetail(pld.Id, session) > 0)
                        {
                            pd.CanDelete = false;
                        }
                        else
                        ...
                        if (pld.ManageCode != null)
                        {
                            pd.ManageCode = new ManageCodeDTO();
                            pd.ManageCode.Id = pld.ManageCode.Id;
                            pd.ManageCode.Name = pld.ManageCode.Name;
                            pd.ManageCode.StaffType = pld.ManageCode.StaffType;
                            if (pld.ManageCode.ProfessorCriterion != null)
                            {
                                pd.ManageCode.ProfessorCriterion = new ProfessorCriterionDTO();
                                pd.ManageCode.ProfessorCriterion.Id = pld.ManageCode.ProfessorCriterion.Id;
                                pd.ManageCode.ProfessorCriterion.Name = pld.ManageCode.ProfessorCriterion.Name;
                                pd.ManageCode.ProfessorCriterion.ManageCode = pld.ManageCode.ProfessorCriterion.ManageCode;
                            }
                            //pd.ManageCode.ProfessorCriterion = pld.ManageCode.ProfessorCriterion != null ? pld.ManageCode.ProfessorCriterion.Map<ProfessorCriterionDTO>() : null;
                        }
                        pd.CriterionDictionary = pld.CriterionDictionary != null ? new CriterionDictionary() { Id = pld.CriterionDictionary.Id, Name = pld.CriterionDictionary.Name } : null;
                    }
                    break;
            }
            return pd;
        ...
        public static PlanKPIMakingDetailDTO ParsePlanDetail(Criterion cri, PlanKPIDetail pld, int type, NHibernate.ISession session, Dictionary<Guid, Guid> listd)
        {
            PlanKPIMakingDetailDTO pd = new PlanKPIMakingDetailDTO();
            switch (type)
            {
                case 1:
                    {
                        ...
                        pd.CreateTime = DateTime.Now;
                        pd.ManageCode = cri.FromPlanKPIDetail.ManageCode != null ? new ManageCodeDTO() { Id = cri.FromPlanKPIDetail.ManageCode.Id } : null;
                        pd.DanhMucMTCL = pld.DanhMucMTCL != null ? new DanhMucMTCLDTO() { Id = pld.DanhMucMTCL.Id, MaDanhMuc = pld.DanhMucMTCL.MaDanhMuc, TenDanhMuc = pld.DanhMucMTCL.TenDanhMuc } : null;
                        ...
                        if (oriParentPlanDetail.Methods != null)
                        {
                            foreach (Method m in oriParentPlanDetail.Methods)
                            {
                                ...
                                method.StartTimeString = m.StartTime.ToString("dd/mm/yyyy");
                                method.EndTimeString = m.EndTime.ToString("dd/mm/yyyy");
                                //   method.leaddepartmentid = m.leaddepartment.id;
                                // method.leaddepartment = m.leaddepartment != null ? new departmentdto() { id = m.leaddepartment.id,name = m.leaddepartment.name } : null;
                                method.LeadDepartment = new List<Method_DepartmentDTO>();
                                foreach (Method_Department de in m.LeadDepartment)
                                {
                ...
                case 1:
                    {
                        ...
                        foreach (ProfessorOtherActivity poa in parentPlanDetail.ProfessorOtherActivities)
                        {
                            ...
                            professor.ExecuteMethod = poa.ExecuteMethod;
                            professor.BasicResource = poa.BasicResource;
                        ...
                        foreach (ScienceResearch sr in parentPlanDetail.ScienceResearches)
                        {
                            ...
                            science.ExecuteMethod = sr.ExecuteMethod;
                            science.BasicResource = sr.BasicResource;
                            science.OrderNumber = GetMaxOrderNumberResearch(planDetail.Id, session) + 1;
                            planDetail.ScienceResearches.Add(science);
                        }

                        PlanKPIDetail planDetail1 = planDetail.Map<PlanKPIDetail>();

                        SessionManager.DoWork(session1 =>
                        {
                            session1.Update(planDetail1);
                        });
                    }
                    break;
                case 2:
                    {
                        ...
                        pld.ProfessorOtherActivities = pld.ProfessorOtherActivities.OrderBy(q => q.OrderNumber).ToList();
                        ...
                        pld.ScienceResearches = pld.ScienceResearches.OrderBy(q => q.OrderNumber).ToList();
                        ...
                        foreach (ProfessorOtherActivity poa in activityList)
                        {
                            ...
                            professor.Id = Guid.NewGuid();
                            professor.NumberOfTime = poa.NumberOfTime;
                            professor.CriterionDictionary = new CriterionDictionary() { Id = poa.CriterionDictionary != null ? poa.CriterionDictionary.Id : Guid.Empty };
                            professor.Name = poa.Name;
                            professor.NumberOfHour = poa.NumberOfHour;
                            professor.ExecuteMethod = poa.ExecuteMethod;
                            professor.BasicResource = poa.BasicResource;
                        ...
                        foreach (ScienceResearch sr in researchList)
                        {
                            ...
                            science.Name = sr.Name;
                            science.NumberOfResearch = sr.NumberOfResearch;
                            science.CriterionDictionary = new CriterionDictionary() { Id = sr.CriterionDictionary != null ? sr.CriterionDictionary.Id : Guid.Empty };
                            science.ExecuteMethod = sr.ExecuteMethod;
                            science.BasicResource = sr.BasicResource;
                            science.OrderNumber = GetMaxOrderNumberResearch(planDetail.Id, session) + 1;
                            planDetail.ScienceResearches.Add(science);
                ...
                case 3: //trường hợp học kỳ chưa soạn kế hoạch
                    {
                        ...
                        pdn.PlanStaff = planStaff != null ? new PlanStaff() { Id = planStaff.Id } : null;
                ...
                case 1:
                    {
                        ...
                        //     pld.CapMucTieu = planDetail.CapMucTieu;
                        ...
                        foreach (ProfessorOtherActivity pa in planDetail.ProfessorOtherActivities)
                        {
                            ...
                            pad.Name = pa.CriterionDictionary != null ? pa.CriterionDictionary.Name : pa.Name;
                            pad.NumberOfTime = pa.NumberOfTime;
                            //pad.NumberOfHour = pa.IsAssign ? pa.NumberOfHour : pa.CriterionDictionary.NumberOfHour;
                            pad.NumberOfHour = pa.NumberOfHour;
                            pad.OrderNumber = pa.CriterionDictionary != null ? pa.CriterionDictionary.OrderNumber : pa.OrderNumber;
                            pad.ExecuteMethod = pa.ExecuteMethod;
                            pad.BasicResource = pa.BasicResource;
        ...
        public static Criterion ParseCriterion(PlanKPIDetail pdn, PlanKPIMakingDetailDTO obj, Guid did, int type, bool isGetStaffLeader)
        {
            Criterion criterion = new Criterion();
            criterion.TargetGroupDetail = new TargetGroupDetail() { Id = obj.TargetGroupDetail.Id };
            criterion.PlanKPI = new PlanKPI() { Id = obj.PlanId };
            criterion.Id = Guid.NewGuid();
            criterion.MaxRecord = obj.MaxRecord;
            criterion.CriterionType = new CriterionType() { Id = 1 };
            criterion.FromPlanKPIDetail = new PlanKPIDetail();
            criterion.FromPlanKPIDetail.Id = obj != null ? obj.Id : Guid.Empty;
            criterion.StaffLeader = isGetStaffLeader ? (obj.StaffLeader != null ? new Staff() { Id = obj.StaffLeader.Id } : null) : null;
            criterion.OrderNumber = obj.OrderNumber; //Kế thừa số thứ tự
            return criterion;
        }

        public static Criterion ParseCriterion(TargetGroupPlanMakingDTO tg, PlanDetailMakingDTO obj, PlanKPIDetail pdn, PlanKPIMakingDetailDTO p, MethodDTO method, Guid did, int type, bool isGetStaffLeader)
        {
            TargetGroupDetail taget = new TargetGroupDetail();
            Criterion criterion = new Criterion();
            criterion.PlanKPI = new PlanKPI() { Id = obj.PlanId };
            criterion.Id = Guid.NewGuid();
            ...
            switch (type)
            {
                ...
                case 2: // khóa xuống tab chi tiết hóa nhiệm vụ
                    {
                        SessionManager.DoWork(session =>
                        {
                            taget = session.Query<TargetGroupDetail>().Where(r => r.TargetGroupDetailType.Id == 7).SingleOrDefault();

                        });
                        criterion.TargetGroupDetail = new TargetGroupDetail() { Id = taget.Id };
                ...
                case 5:
                    ...
                    break;
                case 6: // khóa xuống tab nhận  nhiệm vụ từ cấp trên
                    {
                        SessionManager.DoWork(session =>
                        {
                            taget = session.Query<TargetGroupDetail>().Where(r => r.TargetGroupDetailType.Id == 6).SingleOrDefault();
                        });
                        criterion.TargetGroupDetail = new TargetGroupDetail() { Id = taget.Id };
                        criterion.Name = p.TargetDetail;
                        criterion.FromPlanKPIDetail = new PlanKPIDetail();
                    ...
                    break;
                case 7: // nếu là tab "Khối lượng và hiệu xuất công việc" thì giao xuống tab đó luôn
                    {
                        SessionManager.DoWork(session =>
                        {
                            taget = session.Query<TargetGroupDetail>().Where(r => r.Id == new Guid("1FEC7670-B91E-4E24-AC68-45E270CECED3")).SingleOrDefault();
                        });
                        criterion.TargetGroupDetail = new TargetGroupDetail() { Id = taget.Id };
                        criterion.Name = p.TargetDetail;
                        criterion.FromPlanKPIDetail = new PlanKPIDetail();
        ...
        public static Criterion ParseCriterion(TargetGroupPlanMakingDTO tg, PlanDetailMakingDTO obj, PlanKPIDetail pdn, Guid did, int type, bool isGetStaffLeader)
        {
            TargetGroupDetail taget = new TargetGroupDetail();
            Criterion criterion = new Criterion();
            criterion.PlanKPI = new PlanKPI() { Id = obj.PlanId };
            criterion.Id = Guid.NewGuid();
            criterion.MaxRecord = pdn.MaxRecord;
            criterion.CriterionType = new CriterionType() { Id = 1 };
            criterion.FromPlanKPIDetail = new PlanKPIDetail();
            criterion.FromPlanKPIDetail.Id = pdn != null ? pdn.Id : Guid.Empty;
            criterion.StaffLeader = isGetStaffLeader ? (pdn.StaffLeader != null ? new Staff() { Id = pdn.StaffLeader.Id } : null) : null;
            criterion.OrderNumber = pdn.OrderNumber; //Kế thừa số thứ tự
                ...
                case 2: // khóa xuống tab chi tiết hóa nhiệm vụ
                    {
                        ...
                        criterion.Name = pdn.TargetDetail;
                        criterion.FromPlanKPIDetail = new PlanKPIDetail();
                        criterion.FromPlanKPIDetail.Id = pdn.Id;
                        criterion.FromPlanKPIDetail.PreviousKPI = pdn.PreviousKPI;
                ...
                case 3:
                    {
                        ...
                        criterion.FromPlanKPIDetail.Id = pdn.Id;
                        criterion.FromPlanKPIDetail.PreviousKPI = pdn.PreviousKPI;
                        criterion.FromPlanKPIDetail.CurrentKPI = pdn.CurrentKPI;
                        //criterion.FromPlanKPIDetail.MeasureUnit = p.MeasureUnitDTO != null ? new MeasureUnit() { Id = p.MeasureUnitDTO.Id } : null;
                    }
                    break;
                //update criterion cho đơn vị phối hợp
        ...
        public static PlanKPIDetail CopyPlanKPIDetail(PlanKPIDetail pld)
        {
            ...
            result.ManageCode = pld.ManageCode != null ? new ManageCode() { Id = pld.ManageCode.Id } : null; //pld.ManageCode;
            ...
            result.IsDisable = pld.IsDisable; //Kế thừa trạng thái ngừng sử dụng
            if (pld.ParentPlanKPIDetail != null)
            {
                result.DanhMucMTCL = new DanhMucMTCL() { Id = pld.ParentPlanKPIDetail.DanhMucMTCL.Id };
                result.CapMucTieu = pld.ParentPlanKPIDetail.DanhMucMTCL.CapDanhMuc;
            }
            else if (pld.DanhMucMTCL != null)
            {
                result.DanhMucMTCL = new DanhMucMTCL() { Id = pld.DanhMucMTCL.Id };
                result.CapMucTieu = pld.DanhMucMTCL.CapDanhMuc;
            }
            return result;
        }

        public static int NumberChildPlanKPIDetail(Guid pldId, NHibernate.ISession session)
        {
            List<PlanKPIDetail> childPlanDetails = session.Query<PlanKPIDetail>().Where(p => p.ParentPlanKPIDetail.Id == pldId).ToList();
            return childPlanDetails.Count;
        }
        ...
        public static int GetMaxOrderNumberPlanKPIDetail(Guid planStaffId, Guid targetGroupId, NHibernate.ISession session)
        {
            ...
            list = session.Query<PlanKPIDetail>().Where(p => p.PlanStaff != null && p.TargetGroupDetail != null && !p.IsDelete && p.PlanStaff.Id == planStaffId && p.TargetGroupDetail.Id == targetGroupId).ToList();
            if (list.Count > 0)
        ...
        public static T Clone<T>(this T source)
        {
            ...
            //var deserializeSettings = new JsonSerializerSettings { ReferenceLoopHandling = ReferenceLoopHandling.Ignore };
            var deserializeSettings = new JsonSerializerSettings { ReferenceLoopHandling = ReferenceLoopHandling.Ignore };

            string json = JsonConvert.SerializeObject(source, Formatting.Indented, deserializeSettings);
