...
namespace HRMWeb_Service
{
    ...
    public partial class Service1 : IService1
    {
        ...
        public List<DTO_DangKyKhungGioLamViec> DangKyChamCong_Find(String publicKey, String token, Guid? bophan, Guid ky, int trangthai, bool trangthaidoica)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                CC_DangKyKhungGioLamViec_Factory factory = CC_DangKyKhungGioLamViec_Factory.New();
                List<DTO_DangKyKhungGioLamViec> obj = factory.DangKyChamCong_Find(bophan, ky, trangthai, trangthaidoica);
                return obj;
            }
            else
            {
        ...
        public String DangKyChamCong_Find_Json(String publicKey, String token, Guid? bophan, Guid ky, int trangthai, bool trangthaidoica)
        {//DANG SD
            List<DTO_DangKyKhungGioLamViec> obj = DangKyChamCong_Find(publicKey, token, bophan, ky, trangthai, trangthaidoica);
            String json = JsonConvert.SerializeObject(obj);
            return json;
        }
        ...
        public bool DangKyChamCong_UpdateAll(String publicKey, String token, Guid ky)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                try
                {
                    CC_DangKyKhungGioLamViec_Factory factory = CC_DangKyKhungGioLamViec_Factory.New();
                    //
                    factory.Context.spd_WebChamCong_GanKhungGioLamViec(ky);
                    //
                    return true;
                }
                catch { }
        ...
        public bool QuanLyKhungGio_DoiKhungGio(String publicKey, String token, List<DTO_DangKyKhungGioLamViec> objList)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                try
                {
                    ...
                    foreach (var item in objList)
                    {
                        if (item.Oid != null)
                        {
                            CC_DangKyKhungGioLamViec obj = factory.GetDangKyByOid(item.Oid);
                            if (obj != null && obj.CaChamCongMoi != null)
                                obj.ChoPhepDoi = item.ChoPhepDoi;
                        }
                    }
                    //
                    factory.SaveChanges();
        ...
        public DTO_DangKyKhungGioLamViec DangKyKhungGio_GetDuLieuDaDangKy(String publicKey, String token, Guid idNhanVien,Guid ky)
        {
            DTO_DangKyKhungGioLamViec result = null;
            if (Helper.TrustTest(publicKey, token))
            {
                CC_DangKyKhungGioLamViec_Factory factory = CC_DangKyKhungGioLamViec_Factory.New();
                result = factory.DangKyKhungGio_GetDuLieuDaDangKy(idNhanVien, ky);
            }
            return result;
        }
        ...
        public bool DangKyKhungGioLamViec_Save(String publicKey, String token, DTO_DangKyKhungGioLamViec obj)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                ...
                //1. Đăng ký khung giờ mới
                if (obj.CaChamCong != null && obj.CaChamCongMoi == null)
                {
                    if (objFromDB != null)
                    {
                        objFromDB.CaChamCong = obj.CaChamCong;
                        objFromDB.KyDangKy = obj.KyDangKy;
                        objFromDB.CaChamCongMoi = null;
                        objFromDB.TuNgay = null;
                        objFromDB.DenNgay = null;
                    }
                    else
                    {
                ...
                //2. Đổi khung giờ làm việc
                if (obj.CaChamCong != null && obj.CaChamCongMoi != null)
                {
                    objFromDB.TuNgay = HamDungChung.SetTime(DateTime.Now.AddMonths(1), 2);
                    objFromDB.DenNgay = objFromDB.CC_KyDangKyKhungGio.DenNgay;
                    objFromDB.CaChamCongMoi = obj.CaChamCongMoi;
                }
