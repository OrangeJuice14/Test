...
namespace HRMWebApp.ExcelExport
{
    public class InBangCong : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (context.Request.Params["bophanId"] != null && context.Request.Params["bophanId"] != "undefined")
            {
                bophanId = new Guid(context.Request.Params["bophanId"].ToString());
            }
            ...
            int loaicanbo = context.Request.Params["loaicanbo"] != null ? Convert.ToInt32(context.Request.Params["loaicanbo"].ToString()) : 0;

            //Kiểm tra đã xét ABC chưa
            BoPhan_Factory factory = new BoPhan_Factory();
            if (factory.KiemTraPhongBanDaXetABCVaKhoa_ByDonVi(thang, nam, bophanId))
            {
                BangChamCongExportToExcelProcess(thang, nam, bophanId, idNhanVien, webGroupId, null, Guid.Empty, loaicanbo, context);
            }
        }

        public void BangChamCongExportToExcelProcess(int thang, int nam, Guid bophanId, Guid idNhanVien, string webGroupId, string maNhanSu, Guid idLoaiNhanSu, int loaicanbo, HttpContext context)
        {
            ...
            if (webGroupId.ToUpper() == "53D57298-1933-4E4B-B4C8-98AFED036E21")
                result = factory.QuanLyChamCong_ThongTinChamCongThang_Cua1NhanVien(thang, nam, idNhanVien, loaicanbo);
            else
                result = factory.QuanLyChamCong_ThongTinChamCongThang(thang, nam, bophanId, maNhanSu, idLoaiNhanSu, loaicanbo);
            ...
            for (int i = songay + 3; i <= songay + 7; i++)
            {
            ...
            mergedNgayCong.Value = "Tổng ngày công";
            ...
            //mergedHuongLuong.Value = "Tổng ngày công hưởng lương";
            ...
            WorksheetMergedCellsRegion mergedKhongLuong = sheet.MergedCellsRegions.Add(rowIndex, songay + 4, rowIndex + 2, songay + 4);
            ...
            foreach (DTO_QuanLyChamCong_ThongTinChamCongThang ccThang in result)
            {
                ...
                sheet.Rows[rowIndex].Cells[idx].Value = ccThang.NgayCong + ccThang.TongHuongLuong;
                CellBorder(sheet, rowIndex, idx);
            ...
            sheet.Rows[rowIndex].Cells[17].Value = "Người chấm công";
            sheet.Rows[rowIndex].Cells[17].CellFormat.Alignment = HorizontalCellAlignment.Left;
            sheet.Rows[rowIndex].Cells[17].CellFormat.Font.Bold = ExcelDefaultableBoolean.True;
            ...
            List<DTO_HinhThucNghi> listHinhThucNghi = factoryHTN.GetListForUpdate(loaicanbo).Map<DTO_HinhThucNghi>().ToList();
