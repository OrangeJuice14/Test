...
define(['app/app', 'app/services/kpi/StaffService', 'app/services/kpi/departmentService', 'app/services/kpi/agentObjectService', 'app/services/kpi/ratingKPIService'], function (app) {
    ...
    app.controller('staffController', ['$scope', '$modal', '$rootScope', '$state', '$stateParams', 'staffService', 'departmentService', 'ratingKPIService',
            function ($scope, $modal, $rootScope, $state, $stateParams, staffService, departmentService, ratingKPIService) {
                ...
                $scope.adminDataSource = new kendo.data.DataSource({
                    ...
                    transport: {
                        read: function (options) {
                            return staffService.getListAdminLeader().then(function (result) {
                                options.success(result.data);
                ...
                $scope.EditAdmin = function (Id) {
                    ...
                    var modalInstance = $modal.open({
                        animation: true,
                        templateUrl: '/app/views/kpi/staff/departmentByAdmin_Detail.html',
    ...
    app.controller('adminDetailController', ['$scope', '$modalInstance', 'id', 'staffService', 'departmentService',
        function ($scope, $modalInstance, id, staffService, departmentService) {
            $scope.title = "Đơn vị được quản lý";
            $scope.obj = {};
            staffService.getObj(id).then(function (result) {
                $scope.obj = result.data;
            });
            departmentService.getAdminDepartmentListHierarchy(id).then(function (result) {
