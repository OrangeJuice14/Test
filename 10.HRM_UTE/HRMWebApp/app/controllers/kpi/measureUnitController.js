...
define(['app/app', 'app/services/kpi/criterionService'], function (app) {
    ...
    app.controller('measureUnitController', ['$scope', '$modal', '$rootScope', 'criterionService',
            function ($scope, $modal, $rootScope, criterionService) {
                ...
                $scope.dataSource = new kendo.data.DataSource({
                    transport: {
                        read: function (options) {
                            return criterionService.getListMeasureUnit().then(function (result) {
                ...
                $scope.New = function () {
                    ...
                    var modalInstance = $modal.open({
                        animation: true,
                        templateUrl: '/app/views/kpi/measureUnit/detail.html',
                        controller: 'measureUnitDetailController',
                        resolve: {
                            id: function () {
                                return 0;
                            }
                ...
                $scope.Delete = function (Id) {
                    ...
                    criterionService.getMeasureUnit(Id).then(function (result) {
                        $scope.obj = result.data;
                        criterionService.DeleteMeasure($scope.obj).then(function (result) {
                            if (result == 0)
                            {
                                alert("Đơn vị tính đang được sử dụng!")
                            }
                            $scope.grid.dataSource.read();
                        });
                    });
    ...
    app.controller('measureUnitDetailController', ['$scope', '$modalInstance', 'id', 'criterionService',
        function ($scope, $modalInstance, id,criterionService) {
            $scope.isNew = id == 0 || '' ? true : false;
            $scope.title = "Quản lý đơn vị tính";
            $scope.obj = {};
            if ($scope.isNew) {
                $scope.obj = {
                    Id: 0,
            ...
            $scope.save = function () {
                if ($scope.isNew) {
                    criterionService.SaveMeasure($scope.obj).then(function () {
                        $modalInstance.close();
                    });
                } else {
                    criterionService.SaveMeasure($scope.obj).then(function () {
