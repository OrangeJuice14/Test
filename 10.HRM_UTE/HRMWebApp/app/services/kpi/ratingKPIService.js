define(['app/app'], function (app) {
    ...
    app.factory('ratingKPIService', ['$http', '$q', function ($http, $q) {
        ...
        serviceResult.getListOtherDetail = function (ManageCode) {
            return $http.get('/Api/resultDetailApi/GetListOtherDetail?ManageCode=' + ManageCode);
        };
        serviceResult.getResult = function (id) {
            return $http.get('/Api/resultDetailApi/GetResult?id=' + id);
        }
        serviceResult.getRatingResultDetail = function (id, agentObjectId, planStaffId, supervisorId, departmentId,isAdminRating,isRate) {
            return $http.get('/Api/resultDetailApi/GetRatingResultDetail?planId=' + id + '&agentObjectId=' + agentObjectId + "&planStaffId=" + planStaffId + "&supervisorId=" + supervisorId + "&departmentId=" + departmentId + "&isAdminRating=" + isAdminRating + "&isRate=" + isRate);
        }
        ...
        serviceResult.SaveActivity = function (Obj) {
            ...
            $http({
                method: 'Put',
                url: '/Api/resultDetailApi/PutActivity',
                data: Obj
