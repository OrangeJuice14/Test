<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DetailAll.aspx.cs" Inherits="HRMChamCong.Views.QuanLyChamCong.DetailAll" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $.ajax({
                type: 'POST',
                url: '/Services/ChamCongService.asmx/QuanLyChamCong_ChamCongThangAll',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                data: ko.toJSON({
                    thang: self.thang(),
                ...
                success: function (result) {
                    data = $.parseJSON(result.d);
                    for (var k = 0, lll = data.length; k < lll; k++) {
                        for (var i = 0, l = data[k].ThongTinChamCongThang.length; i < l; i++) {
                            for (var j = 0, ll = days.length; j < ll; j++) {
                                if (data[k].ThongTinChamCongThang[i].ChiTietChamCong.findIndex(q => q.Ngay == days[j].NgayChamCong) === -1) {
                                    data[k].ThongTinChamCongThang[i].ChiTietChamCong.push({
                                        MaHinhThucNghi: '',
                                        KyHieu: '',
                            ...
                            data[k].ThongTinChamCongThang[i].ChiTietChamCong.sort(function (a, b) { return (a.Ngay > b.Ngay) ? 1 : ((b.Ngay > a.Ngay) ? -1 : 0); });
                        }
                    }
                },
                error: function () {
...
<body>
    <div id="chamcong_detail" style="width: 1282px;">
        <table style="font-size: 14pt; font-weight: bold; width: 100%; border: 0px none white">
            <tbody>
                <tr>
                    <%--                    <td style="border: 0px; width:280px;">
                        <img height="50" alt="Trường Đại Học Công Nghiệp TP.HCM" src="/Images/logo.png" />
                    </td>
                    <td style="border:0px;">TRƯỜNG ĐẠI HỌC CÔNG NGHIỆP TP.HCM</td>--%>

                    <td style="border: 0px; text-align: center">
                        <img alt="TRƯỜNG ĐẠI HỌC QUỐC TẾ HỒNG BÀNG" src="/Images/logo_HBU-log-in.png" width="50%" /></td>
                    <td style="font-family: Arial,Tahoma; font-size: 20pt; font-weight: bold; border: 0px">TRƯỜNG ĐẠI HỌC QUỐC TẾ HỒNG BÀNG</td>
                </tr>
            </tbody>
        </table>
        ...
        <div style="margin: 10px 0 10px 0">
                        <table border="0" style="font-family: Arial, Tahoma; font-size: 11pt; padding: 5px;">
                            <tr>
                                <td data-bind="attr: { colspan: $root.spanlength }" style="font-weight: bold; font-size: 14pt; background-color:lightgray">
                                    <span data-bind="text: $data.STT"></span>.
                                    <span data-bind="text: $data.TenBoPhan"></span>
                                </td>
                            </tr>
                            ...
                            <tbody data-bind="foreach: ThongTinChamCongThang">
                                <tr>
