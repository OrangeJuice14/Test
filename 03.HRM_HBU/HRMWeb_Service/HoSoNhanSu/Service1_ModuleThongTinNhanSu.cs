...
namespace HRMWeb_Service
{
    ...
    Guid idNhanVien, Guid kyTinhLuong, byte? loaiLuong)
        ...
        public DTO_ModuleThongTinNhanSu_BANGLUONGNHANVIEN_HBU ModuleThongTinNhanSu_BANGLUONGNHANVIEN_HBU(String publicKey, String token,
   Guid idNhanVien, Guid kyTinhLuong, byte? loaiLuong)
        {
            ...
            {
                var factory = WebUser_Factory.New();
                DTO_ModuleThongTinNhanSu_BANGLUONGNHANVIEN_HBU obj = new DTO_ModuleThongTinNhanSu_BANGLUONGNHANVIEN_HBU();
                try
                {
                    obj = factory.Context.spd_Web1_BangLuongNhanVien_HBU(idNhanVien, kyTinhLuong)
                            .Map<DTO_ModuleThongTinNhanSu_BANGLUONGNHANVIEN_HBU>()
                            .SingleOrDefault();
                    //obj.ThucLanh = obj.LuongTheoNgayCong + obj.LuongNgoaiGio + obj.ThuongLeTet + obj.TruyLuong 
                    //    - obj.ThueTNCN - obj.BHXH - obj.BHYT - obj.BHTN - obj.CongDoan - obj.TruyThu;

                    HoSo hs = factory.Context.HoSoes.Where(h => h.Oid == idNhanVien).SingleOrDefault();
                    if (hs!=null)
                    {
                        obj.MaNhanSu = hs.MaQuanLy;
                        obj.HoTen = hs.HoTen;
                        obj.DonVi = hs.NhanVien.BoPhan1.TenBoPhan;
                    }
                }
                catch (Exception e) {
                }
        ...
        public String ModuleThongTinNhanSu_BANGLUONGNHANVIEN_HBU_Json(String publicKey, String token, Guid idNhanVien, Guid kyTinhLuong, byte? loaiLuong)
        {//DANG SD
            DTO_ModuleThongTinNhanSu_BANGLUONGNHANVIEN_HBU obj = ModuleThongTinNhanSu_BANGLUONGNHANVIEN_HBU(publicKey, token, idNhanVien, kyTinhLuong, loaiLuong);
            String json = JsonConvert.SerializeObject(obj);
            return json;
        }
        #endregion
        ...
        public String ModuleThongTinNhanSu_THONGTINLUONGNHANVIEN_HBU_Json(String publicKey, String token, Guid idNhanVien)
        {//DANG SD
            DTO_ModuleThongTinNhanSu_THONGTINLUONGNHANVIEN obj = ModuleThongTinNhanSu_THONGTINLUONGNHANVIEN_HBU(publicKey, token, idNhanVien);
            String json = JsonConvert.SerializeObject(obj);
            return json;
        }
        public DTO_ModuleThongTinNhanSu_THONGTINLUONGNHANVIEN ModuleThongTinNhanSu_THONGTINLUONGNHANVIEN_LUH(String publicKey, String token, Guid idNhanVien)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                var factory = BangChotThongTinTinhLuong_Factory.New();
                DTO_ModuleThongTinNhanSu_THONGTINLUONGNHANVIEN obj =
                    new DTO_ModuleThongTinNhanSu_THONGTINLUONGNHANVIEN();
                try
                {
        ...
        public DTO_ModuleThongTinNhanSu_THONGTINLUONGNHANVIEN ModuleThongTinNhanSu_THONGTINLUONGNHANVIEN_HBU(String publicKey, String token, Guid idNhanVien)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                var factory = BangChotThongTinTinhLuong_Factory.New();
                ...
                try
                {
                    obj.ThongTinLuong = factory.ThongTinLuong(idNhanVien).Map<DTO_ModuleThongTinNhanSu_ThongTinLuong>();
                }
                catch (Exception) { }
                return obj;
            }
