...
namespace HRMWeb_Service
{
    ...
    public partial class Service1 : IService1
    {
        ...
        public IEnumerable<DTO_HoSoNhanVien> HoSoNhanVienBy_MaBoPhanChamCongNgoaiGio(String publicKey, String token, Guid? maBoPhan)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                HoSo_Factory factory = HoSo_Factory.New();
                IEnumerable<HoSo> tmpList = factory.GetListByMaBoPhan_GCRecordIsNull(maBoPhan).ToList();
                IEnumerable<HoSo> tmpList2 = factory.GetListByMaBoPhan_NhanVienThoiViec_ChamCongNgoaiGio(maBoPhan).ToList();
                var totallist = tmpList.Union(tmpList2);
                IEnumerable<DTO_HoSoNhanVien> list = totallist.Map<DTO_HoSoNhanVien>();
                return list;
            }
            else
            {
        ...
        public String GetList_HoSoNhanVienBy_MaBoPhanChamCongNgoaiGio_Json(String publicKey, String token, Guid? maBoPhan)
        {//DANG SD
            IEnumerable<DTO_HoSoNhanVien> list = HoSoNhanVienBy_MaBoPhanChamCongNgoaiGio(publicKey, token, maBoPhan);
            String json = JsonConvert.SerializeObject(list);
            return json;
        }
        public IEnumerable<DTO_HoSoNhanVien> GetList_HoSoNhanVien_ByCurrentUser(String publicKey, String token, Guid idNhanVien)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                HoSo_Factory factory = HoSo_Factory.New();
                IEnumerable<DTO_HoSoNhanVien> list = factory.GetListByIdNV_GCRecordIsNull(idNhanVien).ToList();
                return list;
            }
            else
            {
        ...
        public String GetList_HoSoNhanVien_ByCurrentUser_Json(String publicKey, String token, Guid idNhanVien)
        {//DANG SD
            IEnumerable<DTO_HoSoNhanVien> list = GetList_HoSoNhanVien_ByCurrentUser(publicKey, token, idNhanVien);
            String json = JsonConvert.SerializeObject(list);
            return json;
        }
    }
