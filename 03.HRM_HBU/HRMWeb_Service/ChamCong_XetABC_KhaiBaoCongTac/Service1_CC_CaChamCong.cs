...
namespace HRMWeb_Service
{
    ...
    public partial class Service1 : IService1
    {
        ...
        public bool DangKyChamCong_Create(String publicKey, String token, Guid ca)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                CC_DangKyKhungGioLamViec_Factory factory = CC_DangKyKhungGioLamViec_Factory.New();
                HoSo_Factory hsfact = HoSo_Factory.New();
                //Lấy list đã đăng ký
                var listDaDangKy = factory.GetListAllOid();
                //Lấy list tất cả hiện tại
                var lisths = hsfact.GetListOidAll_GCRecordIsNull();
                //Lấy list chưa đăng ký
                var list = lisths.Where(p => !listDaDangKy.Any(p2 => p2 == p));
                foreach (Guid id in list)
                {
                    CC_DangKyKhungGioLamViec cc = factory.CreateManagedObject();
                    cc.Oid = Guid.NewGuid();
                    cc.ThongTinNhanVien = id;
                    cc.CaChamCong = ca;
                    cc.KyDangKy = new Guid("00000000-0000-0000-0000-000000000001");
                }
                factory.SaveChanges();
                return true;
            }
        ...
        public bool DangKyChamCong_DoiCa(String publicKey, String token, Guid ca, Guid? bophan, byte loai,List<DTO_DangKyKhungGioLamViec> list)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                CC_DangKyKhungGioLamViec_Factory factory = CC_DangKyKhungGioLamViec_Factory.New();
                ...
                switch (loai)
                {
                    case 1:
                        {
                            //Lấy list đã đăng ký
                            var listDaDangKy = factory.GetListAll();
                            foreach (CC_DangKyKhungGioLamViec cc in listDaDangKy)
                            {
                                cc.CaChamCong = ca;
                            }
                            factory.SaveChanges();
                    ...
                    case 2:
                        {
                            Guid idbophan = bophan ?? Guid.Empty;
                            //Lấy list đã đăng ký theo đơn vị
                            var listDaDangKy = factory.GetListByBoPhan(idbophan);
                            foreach (CC_DangKyKhungGioLamViec cc in listDaDangKy)
                            {
                                cc.CaChamCong = ca;
                            }
                    ...
                    case 3:
                        {
                            //Lấy list đã đăng ký cá nhân
                            List<Guid> listOid = list.Select(s => s.Oid).ToList();
                            var listDaDangKy = factory.GetListByListOid(listOid);
                            foreach (CC_DangKyKhungGioLamViec cc in listDaDangKy)
                            {
                                cc.CaChamCong = ca;
                            }
        ...
        public bool DangKyChamCong_DoiCaLinhDong_SaveList(String publicKey, String token,
            ...
            foreach (var thongTinChamCongThang in objList)
            {
                if (thongTinChamCongThang.ChiTietChamCong != null)
                {
                    foreach (var obj in thongTinChamCongThang.ChiTietChamCong)
                    {
                        ...
                        if (objFromDbForSave != null && obj.CC_CaChamCong != (objFromDbForSave.CC_CaChamCong ?? Guid.Empty))
                        {
                            if (obj.CC_CaChamCong != Guid.Empty)
                                objFromDbForSave.CC_CaChamCong = obj.CC_CaChamCong;
                            else
                                objFromDbForSave.CC_CaChamCong = null;
                        }
                    }
        ...
        public bool DangKyChamCong_DoiCaLinhDong_SaveList_Json(String publicKey, String token, string jsonObjectList)
        {//DANG SD
            ...
            return DangKyChamCong_DoiCaLinhDong_SaveList(publicKey, token, objList);
        }
        ...
        public DTO_CC_CaChamCong CaChamCong_GetByID(String publicKey, String token, Guid id)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                ...
                obj.GioRaThu7 = ParseHourDTO(obj.ThoiGianRaThu7);
                obj.PhutRaThu7 = ParseMinuteDTO(obj.ThoiGianRaThu7);
                return obj;
            }
            else
            {
        ...
        public List<DTO_DangKyKhungGioLamViec> DangKyChamCong_Find(String publicKey, String token, Guid? bophan)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                CC_DangKyKhungGioLamViec_Factory factory = CC_DangKyKhungGioLamViec_Factory.New();
                List<DTO_DangKyKhungGioLamViec> obj = factory.DangKyChamCong_FindAll(bophan);
                return obj;
            }
            else
            {
        ...
        public String DangKyChamCong_Find_Json(String publicKey, String token, Guid? bophan)
        {//DANG SD
            List<DTO_DangKyKhungGioLamViec> obj = DangKyChamCong_Find(publicKey, token, bophan);
            String json = JsonConvert.SerializeObject(obj);
            return json;
        }
            ...
            if (Helper.TrustTest(publicKey, token))
            {
                ...
                if (Oid != Guid.Empty)
                {
                    ...
                    if (obj != null)
                    {
                        ...
                        obj.ThoiGianRaThu7 = ParseTimeString(GioRaThu7, PhutRaThu7);
        ...
        public bool DangKyKhungGioLamViec_Save(String publicKey, String token, DTO_DangKyKhungGioLamViec obj)
        {
            if (Helper.TrustTest(publicKey, token))
            {
                ...
                objFromDB = factory.GetDangKyByKy(obj.KyDangKy);
                if (objFromDB != null)
                {
        ...
        public CC_KyDangKyKhungGio KyDangKyKhungGio_New(String publicKey, String token, Guid id, String tenky, DateTime tuNgay, DateTime denNgay)
        {//DANG SD
            if (Helper.TrustTest(publicKey, token))
            {
                CC_KyDangKyKhungGio_Factory factory = CC_KyDangKyKhungGio_Factory.New();
        ...
        {//DANG SD
            ...
            if (Helper.TrustTest(publicKey, token))
            {
                ...
                //var result = (from o in factory.Context.CC_KyDangKyKhungGio
                //                                       where (isNew || o.Oid != Oid)
                //                                           && (
