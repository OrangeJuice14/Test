...
namespace HRMWebApp.KPI.Core.Controllers
{
    public class PlanKPIDetailApiController : ApiController
    {
        ...
        //    SessionManager.DoWork(session =>
        //    {
        //        session.Query<Method>().Cacheable().CacheMode(NHibernate.CacheMode.Normal).ToList();
        //        //session.Query<PlanKPIDetail>().Cacheable<PlanKPIDetail>().CacheMode(NHibernate.CacheMode.Normal);
        //        if (isSupervisor == 1 || AuthenticationHelper.IsAllowPlanMaking(new Guid(HttpContext.Current.User.Identity.GetUserId()), NormalStaffId != Guid.Empty ? NormalStaffId : new Guid(HttpContext.Current.User.Identity.GetUserId())))
        //        {
        //            //Preparing data
        ...
        //                                {
        //                                    List<MethodDTO> originalMethods = ControllerHelpers.GetOriginalMethods(cri.FromPlanKPIDetail, session);
        ...
        //                                    bool isValid = cri.FromPlanKPIDetail != null && originalMethods.Count > 0 && !cri.FromPlanKPIDetail.IsDelete
        ...
        //                                        {
        //                                            List<MethodDTO> originalMethods = ControllerHelpers.GetOriginalMethods(pd, session);
        ...
        //                                staffPlanCriterions = temstaffPlanCriterions.Where(c =>
        //                                c.FromPlanKPIDetail != null && ControllerHelpers.GetOriginalMethods(c.FromPlanKPIDetail.Id, session).Count > 0
        //                                    //(c.FromPlanKPIDetail == null || !c.FromPlanKPIDetail.IsDisable && ControllerHelpers.GetOriginalMethods(c.FromPlanKPIDetail.Id, session).Count > 0)
        //                                    && ((plan.StartTime.CompareTo(ControllerHelpers.GetOriginalMethods(c.FromPlanKPIDetail.Id, session).Min(m => m.StartTime)) >= 0 && plan.StartTime.CompareTo(ControllerHelpers.GetOriginalMethods(c.FromPlanKPIDetail.Id, session).Max(m => m.EndTime)) <= 0) ||
        //                                                  (plan.EndTime.CompareTo(ControllerHelpers.GetOriginalMethods(c.FromPlanKPIDetail.Id, session).Min(m => m.StartTime)) >= 0 && plan.EndTime.CompareTo(ControllerHelpers.GetOriginalMethods(c.FromPlanKPIDetail.Id, session).Max(m => m.EndTime)) <= 0) ||
        //                                                  (plan.StartTime.CompareTo(ControllerHelpers.GetOriginalMethods(c.FromPlanKPIDetail.Id, session).Min(m => m.StartTime)) <= 0 && plan.EndTime.CompareTo(ControllerHelpers.GetOriginalMethods(c.FromPlanKPIDetail.Id, session).Max(m => m.EndTime)) >= 0))).ToList();
        ...
        //                                                if (planDetails.First().TargetDetail == null)
        //                                                {
        //                                                    pd = ControllerHelpers.ParsePlanDetail(null, staffPlanCriterion.FromPlanKPIDetail, 2, session);
        //                                                }
        ...
        //                                                    PlanKPIDetailDTO parentpl = ControllerHelpers.GetParentPlanKPIDetail(pld, session);
        //                                                    foreach (PlanKPIDetail_KPIDTO kpi in parentpl.PlanKPIDetail_KPIs)
        //                                                    {
        //                                                        PlanKPIDetail_KPIDTO kd = new PlanKPIDetail_KPIDTO();
        //                                                        kd.Id = kpi.Id;
        ...
        //                                        childPlan.Id = Guid.NewGuid();
        //                                        childPlan.PlanKPIDetail_KPIs = new List<PlanKPIDetail_KPI>();
        //                                        childPlan.StaffLeader =cri.FromPlanKPIDetail.StaffLeader!=null? new Staff { Id = cri.FromPlanKPIDetail.StaffLeader.Id } : new Staff { Id = staff.Id };
        //                                        pd.BasicResource = string.Empty;
        //                                        pd.ParentPlanKPIDetail_KPIs = new List<PlanKPIDetail_KPIDTO>();
        //                                        foreach (PlanKPIDetail_KPI kpi in childPlan.ParentPlanKPIDetail.PlanKPIDetail_KPIs)
        //                                        {
        ...
        //                                    //Kế hoạch phân thêm cho GV được thêm vào hoạt động khác của từng nhóm
        //                                    if (cri.TargetGroupDetail.TargetGroupDetailType.Id == 0)
        //                                    {
        ...
        //                                                {
        //                                                    if (pldto.CriterionTypeId == 1) //Các hoạt động giảng dạy khác (Loại điền chữ)
        //                                                    {
        //                                                        PlanKPIDetail p = session.Query<PlanKPIDetail>().Where(pp => pp.Id == pldto.Id).SingleOrDefault();
        //                                                        string[] name = { };
        //                                                        if (p.Name != null)
        //                                                        {
        //                                                            name = p.Name.Split('\r');
        ...
        //                                                        {
        //                                                            if (s == childPlan.TargetDetail) //tìm xem hoạt động này đã được kế thừa từ năm chưa
        //                                                            {
        //                                                                count++; //nếu rồi thì tăng biến đếm
        //                                                            }
        //                                                        }
        //                                                        if (count == 0) // nếu hoạt động chưa được kế thừa thì add vào (không có hoạt động nào trùng tên)
        //                                                        {
        //                                                            if ((p.Name != "" || p.ExecuteMethod != "" || p.BasicResource != "" || p.PreviousKPI != "" || p.CurrentKPI != "") && (p.Name != null || p.ExecuteMethod != null || p.BasicResource != null || p.PreviousKPI != null || p.CurrentKPI != null))
        //                                                            {//Nếu hoạt động đã có dữ liệu thì thêm ký tự xuống dòng
        //                                                                p.Name += (char)13 + childPlan.TargetDetail;
        //                                                                p.ExecuteMethod += cri.FromPlanKPIDetail.Methods.Count > 0 ? (char)13 + cri.FromPlanKPIDetail.Methods.First().Name : (char)13 + "";
        //                                                                p.BasicResource += (char)13 + childPlan.BasicResource;
        //                                                                p.PreviousKPI += (char)13 + childPlan.PreviousKPI;
        //                                                                p.CurrentKPI += childPlan.PlanKPIDetail_KPIs.Count > 0 ? (char)13 + childPlan.PlanKPIDetail_KPIs.First().Name : (char)13 + "";
        //                                                            }
        //                                                            else
        //                                                            {
        //                                                                p.Name += childPlan.TargetDetail;
        //                                                                p.ExecuteMethod += cri.FromPlanKPIDetail.Methods.Count > 0 ? cri.FromPlanKPIDetail.Methods.First().Name : "";
        //                                                                p.BasicResource += childPlan.BasicResource;
        //                                                                p.PreviousKPI += childPlan.PreviousKPI;
        //                                                                p.CurrentKPI += childPlan.PlanKPIDetail_KPIs.Count > 0 ? childPlan.PlanKPIDetail_KPIs.First().Name : "";
        //                                                            }
        //                                                            session.Merge(p);

        //                                                            //hiển thị lên giao diện
        //                                                            pldto.Name = p.Name;
        //                                                            pldto.ExecuteMethod = p.ExecuteMethod;
        //                                                            pldto.BasicResource = p.BasicResource;
        //                                                            pldto.PreviousKPI = p.PreviousKPI;
        //                                                            pldto.CurrentKPI = p.CurrentKPI;
        //                                                        }
        //                                                    }
        //                                                }
        //                                            }
        ...
        //                                    if (cri.TargetGroupDetail.TargetGroupDetailType.Id == 4) //Nhóm hoạt động khác
        //                                    {
        //                                        foreach (TargetGroupPlanMakingDTO tg in result.TargetGroupPlanMakings)
        //                                        {
        ...
        //                                                foreach (PlanKPIMakingDetailDTO pldto in tg.PlanKPIDetails)
        //                                                {
        //                                                    if (pldto.CriterionTypeId == 5) //Hoạt động khác
        //                                                    {
        //                                                        List<ProfessorOtherActivityDTO> activityList = new List<ProfessorOtherActivityDTO>();
        //                                                        activityList = pldto.ProfessorOtherActivities.Where(p => p.Name == childPlan.TargetDetail).ToList();
        //                                                        if (activityList.Count == 0)
        //                                                        {
        //                                                            ProfessorOtherActivity pa = new ProfessorOtherActivity();
        //                                                            pa.Id = Guid.NewGuid();
        //                                                            pa.Name = childPlan.TargetDetail;
        //                                                            pa.NumberOfTime = 1;
        //                                                            pa.NumberOfHour = 1;
        //                                                            pa.CriterionDictionary = null;
        //                                                            pa.PlanKPIDetail = new PlanKPIDetail() { Id = pldto.Id };
        //                                                            pa.OrderNumber = ControllerHelpers.GetMaxOrderNumberActivity(pldto.Id, session) + 1;

        //                                                            session.Save(pa);
        //                                                            pldto.ProfessorOtherActivities.Add(pa.Map<ProfessorOtherActivityDTO>()); //hiển thị hoạt động mới
        //                                                        }
        //                                                    }
        //                                                }
        //                                            }
        ...
        //                                    if (cri.TargetGroupDetail.TargetGroupDetailType.Id == 5) //Nhóm hoạt động nghiên cứu khoa học
        //                                    {
        //                                        foreach (TargetGroupPlanMakingDTO tg in result.TargetGroupPlanMakings)
        //                                        {
        ...
        //                                                    if (pldto.CriterionTypeId == 5) //Hoạt động khác
        //                                                    {
        //                                                        List<ScienceResearchDTO> researchList = new List<ScienceResearchDTO>();
        //                                                        researchList = pldto.ScienceResearches.Where(p => p.Name == childPlan.TargetDetail).ToList();
        //                                                        if (researchList.Count == 0)
        //                                                        {
        //                                                            ScienceResearch sr = new ScienceResearch();
        //                                                            sr.Id = Guid.NewGuid();
        //                                                            sr.Name = childPlan.TargetDetail;
        //                                                            sr.NumberOfResearch = 1;
        //                                                            sr.CriterionDictionary = null;
        //                                                            sr.PlanKPIDetail = new PlanKPIDetail() { Id = pldto.Id };
        //                                                            sr.OrderNumber = ControllerHelpers.GetMaxOrderNumberResearch(pldto.Id, session) + 1;

        //                                                            session.Save(sr);
        //                                                            pldto.ScienceResearches.Add(sr.Map<ScienceResearchDTO>()); //hiển thị hoạt động mới
        //                                                        }
        //                                                    }
        //                                                }
        //                                            }
        ...
        //                                    pd.StaffLeader.Id = subjectLeader.Id;
        //                                    pd.StaffLeader.Name = subjectLeader.StaffProfile != null ? subjectLeader.StaffProfile.Name : "";
        //                                    //Thêm tên nhóm mục tiêu
        //                                    pd.TargetGroupDetailOrderNumber = pld.FromCriterion.TargetGroupDetail.OrderNumber;
        //                                    result.AdditionalPlanDetails.Add(pd);
        //                                }
        //                            }
        //                        }
        ...
        //                                            if (leadCriterions.Count <= 0 && p.StaffLeader != null)
        //                                            {
        ...
        //                                            {
        //                                                if (p.StaffLeader != null && sid != p.StaffLeader.Id)
        //                                                {
        ...
        //                            }
        //                            //Cập nhật công việc đc phân công cho đơn vị
        //                            UpdateSubject(plDetail, p, session);
        //                            //Những phó khoa được phân công công việc
        //                            foreach (Guid sid in p.SubStaffIds)
        //                            {
