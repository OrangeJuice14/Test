...
define(['app/app', 'app/services/kpi/planKPIService', 'app/services/kpi/StaffService', 'app/services/kpi/agentObjectService', 'app/services/kpi/ratingKPIService', 'app/services/kpi/departmentService', 'moment'], function (app) {
    ...
    app.controller('planManageDetailController', ['$rootScope','$scope', '$modalInstance', 'id', 'planType', 'ratingStartTime', 'ratingEndTime', '$stateParams', 'planKPIService', 'agentObjectService', 'staffService', 'ratingKPIService',
        function ($rootScope,$scope, $modalInstance, id, planType, ratingStartTime, ratingEndTime, stateParams, planKPIService, agentObjectService, staffService, ratingKPIService) {
            var moment = require('moment');
            $scope.date = new Date();
           ...
           $scope.DepartmentManagePlan = function (Id) {
               if (Id == '406f6e70-c3d9-49d8-b6f6-e39649ba8458')
               {
                       staffService.getCurrentStaff().then(function (result2) {
                           ...
                           $rootScope.isSupervisor = 1;
                           $modalInstance.close();
                           $state.go("schoolManagePlankpidetail", { planId: $scope.planId, agentObjectId: "4731710F-94FB-4EF5-9F48-05E12648AA62", normalStaffId: $scope.normalStaffId, isSupervisor: 1 });
               ...
               else
               {
                   ...
                   staffService.getDepartmentLeaderId(Id).then(function (result2) {
                       ...
                       $modalInstance.close();
                       $rootScope.isSupervisor = 1;
                       $state.go("departmentManagePlankpidetail", { planId: $scope.planId, agentObjectId: $scope.agentObjectId, normalStaffId: $scope.normalStaffId, isSupervisor: 1, isConfig: 0 });
           ...
           $scope.FacultyManagePlan = function (Id) {
               staffService.getDepartmentLeaderAgentObjectId(Id).then(function (result) {
                   ...
                   staffService.getDepartmentLeaderId(Id).then(function (result2) {
                       ...
                       $modalInstance.close();
                       $rootScope.isSupervisor = 1;
                       $state.go('facultyManagePlankpidetail', { planId: $scope.planId, agentObjectId: $scope.agentObjectId, normalStaffId: $scope.normalStaffId, isSupervisor: 1, isConfig: 0 })
