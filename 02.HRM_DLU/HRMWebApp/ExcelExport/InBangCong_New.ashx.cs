...
{
    public class InBangCong_New : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            int loaiCanBo = context.Request.Params["loaicanboId"] != null ? Convert.ToInt32(context.Request.Params["loaicanboId"].ToString()) : 0;
            //
            BangChamCongExportToExcelProcess(thang, nam, loaiCanBo, context);
        }

        public void BangChamCongExportToExcelProcess(int thang, int nam, int loaicanbo, HttpContext context)
        {
            ...
            result = factory.QuanLyChamCong_ThongTinChamCongThang_New(thang, nam, loaicanbo);
            //
            BoPhan_Factory bpfac = new BoPhan_Factory();
            string captionHeader = string.Empty;
            if (loaicanbo == 0)
                captionHeader = "TOÀN TRƯỜNG";
            else if (loaicanbo == 2)
                captionHeader = "NHÂN VIÊN HÀNH CHÍNH";
            else
                captionHeader = "GIẢNG VIÊN";
            ...
            string str = string.Format("{0}_{1}_{2}_{3}_{4}_{5}_{6}", captionHeader, now.Day, now.Month, now.Year, now.Hour, now.Minute, now.Millisecond);
            str = str + ".xls";
            ...
            merged5.Value = captionHeader.ToUpper();
            ...
            //
            WorksheetMergedCellsRegion mergedDonVi = sheet.MergedCellsRegions.Add(rowIndex, 0, rowIndex + 2, 0);
            ...
            {
                sheet.Rows[rowIndex].Cells[0].Value = ccThang.TenDonVi;
