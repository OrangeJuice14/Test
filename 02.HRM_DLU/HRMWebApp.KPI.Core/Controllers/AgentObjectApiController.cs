...
using HRMWeb_Business.Model.MapObjectCreator;
...
namespace HRMWebApp.KPI.Core.Controllers
{
    public class AgentObjectApiController : ApiController
    {
        ...
        public int PutThuNhapKhac(LoaiThuNhapKhacDTO obj)
        {
            OtherIncome loaiTNK = new OtherIncome();
            SessionManager.DoWork(session =>
            {
                loaiTNK.Oid = Guid.NewGuid();
                loaiTNK.MaQuanLy = obj.MaQuanLy;
                loaiTNK.TenLoaiThuNhapKhac = obj.TenLoaiThuNhapKhac;
                session.Save(loaiTNK);
            });
            return 1;
        }
        ...
        public IEnumerable<KyTinhLuongDTO> GetList_KyTinhLuong()
        {
            List<KyTinhLuongDTO> result = new List<KyTinhLuongDTO>();
            KyTinhLuong_Factory factory = KyTinhLuong_Factory.New();
            result = factory.GetAll_GCRecordIsNull().Map<KyTinhLuongDTO>();
            foreach (KyTinhLuongDTO ky in result)
            {
                string thang = ky.Thang < 10 ? "0" + ky.Thang.ToString() : ky.Thang.ToString();
                ky.Name ="Tháng " + thang + "/" + ky.Nam.ToString();
            }
            return result;
        }
    }
