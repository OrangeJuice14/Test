...
namespace HRMWeb_Service
{
    ...
    public partial class Service1 : IService1
    {
                                    ...
                                    TimeSpan.FromSeconds(360)))
                {
                    ...
                    foreach (var obj in objList)
                    {
                        if (obj != null)
                        {
                            ...
                            var trangThaiFromDB = objFromDB.TrangThai;
                            if (objFromDB != null)
                            {
                                if (trangThai == 1 && objFromDB.TrangThai != 1)
                                {
                                    objFromDB.TrangThai = trangThai;
                                    factory.SaveChanges();
                                    factory.Context.spd_WebChamCong_CC_KhaiBaoCongTac_Duyet(objFromDB.Oid);
                                }
                                else if (trangThai == 0) //chỉ hủy duyệt khi trước đó đã duyệt (-1: chờ xét)
                                {
                                    objFromDB.TrangThai = trangThai;
                                    factory.SaveChanges();
                                    if (trangThaiFromDB == 1)
                                        factory.Context.spd_WebChamCong_CC_KhaiBaoCongTac_HuyDuyet(objFromDB.Oid);
                                }
                            }
        ...
        public int QuanLyKhaiBaoCongTac_LaySoKhaiBaoCongTacDangChoXet(String publicKey, String token, int thang, int nam, Guid boPhanId, Guid webUserId)
        {
            var result = 0;
            if (Helper.TrustTest(publicKey, token))
            {
                CC_KhaiBaoCongTac_Factory factory = CC_KhaiBaoCongTac_Factory.New();
                result = factory.LaySoKhaiBaoCongTacDangChoXet(thang, nam, boPhanId, null, webUserId);
            }
