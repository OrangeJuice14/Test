    ...
    public class ABC_EvaluationBoardApiController : ApiController
    {
        ...
        public IEnumerable<ABC_EvaluationBoardDTO> GetList() //tab đánh giá cấp dưới
        {
            ...
            SessionManager.DoWork(session =>
            {
                ...
                if (session.Query<Staff>().Any(s => s.Department != null && s.Department.Id == staff.DepartmentId && s.StaffInfo.StaffType.ManageCode == "NV"))
                {
        ...
        public List<ABC_EvaluationBoardStaffDTO> GetListStaffSyntheticEvaluationExcel(Guid evaluationId, Guid departmentId, bool isGet1)
        {
            ...
            SessionManager.DoWork(session =>
            {
                var evaluationFromDate = session.Query<ABC_EvaluationBoard>().Where(e => e.Id == evaluationId).Select(q => q.FromDate).SingleOrDefault();
                //Lấy trưởng đơn vị
                Staff staff = session.Query<Staff>().Where(s => s.Department.Id == departmentId && s.StaffInfo.Position != null && s.StaffInfo.WebUser.WebGroupId == Guid.Parse("00000000-0000-0000-0000-000000000004")).FirstOrDefault();
                //Lấy staff trừ trưởng đơn vị
        ...
        public IEnumerable<ABC_EvaluationBoardStaffDTO> GetListDepartmentLeaderSyntheticEvaluation(Guid evaluationId)
        {
            ...
            SessionManager.DoWork(session =>
            {
                ...
                var deptLeaderList = session.Query<Staff>().Where(s => (s.StaffStatus.NoLongerWork == 0 || s.InactivityDate > evaluationFromDate) && s.StaffInfo.Position != null && s.StaffInfo.WebUser.WebGroupId == Guid.Parse("00000000-0000-0000-0000-000000000004")).OrderBy(s => s.Department.OrderNumber);
        ...
        public IEnumerable<ABC_EvaluationBoardStaffDTO> GetListDepartmentLeaderEvaluation(Guid evaluationId)
        {
            ...
            SessionManager.DoWork(session =>
            {
                var evaluationFromDate = session.Query<ABC_EvaluationBoard>().Where(e => e.Id == evaluationId).Select(q => q.FromDate).SingleOrDefault();
                //var deptLeaderList = session.Query<Staff>().Where(s => s.StaffInfo.Position != null && !s.StaffInfo.Position.Name.Contains("Phó") && !s.StaffInfo.Position.Name.Contains("bộ môn") && !s.StaffInfo.Position.Name.Contains("Hiệu") && !s.StaffInfo.Position.Name.Contains("Tổ")).OrderBy(s => s.Department.OrderNumber);
                var departmentLeaderWebUsers = session.Query<DB.Entities.WebUser>().Where(q => q.WebGroupId == Guid.Parse("00000000-0000-0000-0000-000000000004"));
                var deptLeaderList = session.Query<Staff>().Where(s => (s.StaffStatus.NoLongerWork == 0 || s.InactivityDate > evaluationFromDate) && s.StaffInfo.Position != null && (departmentLeaderWebUsers.Any(qq => qq.StaffInfo.Id == s.StaffInfo.Id) || s.StaffInfo.Position.LaQuanLy)).OrderBy(s => s.Department.OrderNumber);
                var currentStaffId = GetCurrentUser().Id;
                Staff staff = session.Query<Staff>().SingleOrDefault(s => s.Id == currentStaffId);
                List<Staff> staffList = new List<Staff>();
        ...
        public int GetCreateEvaluationBoard(int year)
        {
            try
            {
                ...
                SessionManager.DoWork(session =>
                {
                    ...
                    evYear.StartRating = evYear.FromDate;
                    evYear.EndRating = evYear.ToDate;
                    ...
                    for (int j = 0; j <= 11; j++)
                    {
                        ABC_ClassEvaluationBoard evMonth = new ABC_ClassEvaluationBoard();
                        evMonth.Id = Guid.NewGuid();
                        evMonth.FromDate = evYear.FromDate.AddMonths(j);
                        evMonth.ToDate = evMonth.FromDate.AddMonths(1).AddDays(-1);
                        evMonth.Name = "Tháng " + (j + 1).ToString() + "/" + year.ToString();
                        evMonth.Month = j + 1;
                        evMonth.Year = year;
                        session.Save(evMonth);
                    }
                });
                return result;
            }
            ...
            {
                Helper.ErrorLog("ABC_EvaluationBoardApi/GetCreateEvaluationBoard", ex);
                throw;
            }
        }
        ...
        {
            int result = 0;
            if (staff.StaffInfo?.StaffType?.ManageCode != null && staff.Department != null && staff.Department.ManageCode != null)
            {
                if (!staff.StaffInfo.StaffType.ManageCode.Contains("NV"))
                {
                    result = 1;
                }
                else result = 2;
        ...
        public ABC_RatingTypeDTO GetRatingTypeForEvaluationBoard(Staff staff, NHibernate.ISession session)
        {
            ABC_RatingTypeDTO ratingTypeDTO = new ABC_RatingTypeDTO();
            try
            {
                ThongTinDanhGia obj = session.Query<ThongTinDanhGia>().SingleOrDefault(q => q.StaffInfo.Id == staff.Id);
                if (obj != null)
                {
        ...
        [Authorize]
        [Route("")]
        public IEnumerable<ABC_EvaluationBoardStaffDTO> GetListStaffByDepartment(Guid departmentId)
        {
            List<ABC_EvaluationBoardStaffDTO> evaluationBoardStaffDTOs = new List<ABC_EvaluationBoardStaffDTO>();
            SessionManager.DoWork(session =>
            {
                var temp = session.Query<Staff>().Where(s => s.Department.Id == departmentId && (s.StaffStatus.NoLongerWork == 0));
                temp = temp.OrderByDescending(s => s.StaffInfo.Position.HSPCChucVu).ThenBy(s => s.Title.Name).ThenBy(s => s.StaffProfile.FirstName).ThenBy(s => s.StaffProfile.Name);
                int stt = 0;
                foreach (Staff st in temp)
                {
                    ...
                    if (st.StaffInfo != null)
                    {
                        ...
                        std.StaffType = st.StaffInfo?.StaffType != null ? (st.StaffInfo.StaffType.Name.Contains("Giảng") ? 1 : 2) : 0;
                        var ttdg = session.Query<ThongTinDanhGia>().Where(t => t.StaffInfo.Id == st.Id).SingleOrDefault();
                        if (ttdg != null)
                        {
                            std.RatingType = ttdg.ABC_RatingType.Name;
                        }
                        stt++;
                        std.OrderNumber = stt;
                        evaluationBoardStaffDTOs.Add(std);
                    }
                }
            });
            return evaluationBoardStaffDTOs;
        }
        ...
        [Authorize]
        [Route("")]
        public bool GetCheckTimeRating(Guid evaluationId)
        {
            ...
            SessionManager.DoWork(session =>
            {
                // Lấy kỳ đánh giá
                ABC_EvaluationBoard eb = session.Query<ABC_EvaluationBoard>().Where(e => e.Id == evaluationId).SingleOrDefault();
                if (eb != null && eb.StartRating <= DateTime.Now && DateTime.Now <= eb.EndRating)
                {
                    result = true;
                }
            });
