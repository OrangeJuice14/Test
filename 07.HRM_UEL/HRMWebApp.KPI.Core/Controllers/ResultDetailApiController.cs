                                    ...
                                    case (int)AgentObjectTypes.GiangVien:
                                        {
                                            ...
                                            else
                                            {
                                                foreach (PlanKPIDetail pld in planDetails)
                                                {
                                                    ...
                                                    rd.ProfessorOtherActivities = rd.ProfessorOtherActivities.OrderBy(p => p.OrderNumber).ToList();
                                    ...
                                    case (int)AgentObjectTypes.NhanVien:
                                        {
                                            ...
                                            foreach (PlanKPIDetail pld in planDetails)
                                            {
                                                ...
                                                if (tg.TargetGroupDetailTypeId == 3)
                                                {
                                                    //if (ControllerHelpers.GetOriginalParentPlanKPIDetail(pld.Id, session).TargetDetail != null)
                                                    //    rd.PlanKPIDetailName = ControllerHelpers.GetOriginalParentPlanKPIDetail(pld.Id, session).TargetDetail;
                                                    //else rd.PlanKPIDetailName = pld.TargetDetail;
                                                    if (ControllerHelpers.GetOriginalParentPlanKPIDetailTest(pld.Id).TargetDetail != null)
                                                        rd.PlanKPIDetailName = ControllerHelpers.GetOriginalParentPlanKPIDetailTest(pld.Id).TargetDetail;
                                                    else rd.PlanKPIDetailName = pld.TargetDetail;
                                                }
                                                else rd.PlanKPIDetailName = pld.TargetDetail;
