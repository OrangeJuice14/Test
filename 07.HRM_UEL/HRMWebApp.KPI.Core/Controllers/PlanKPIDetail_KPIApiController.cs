...
namespace HRMWebApp.KPI.Core.Controllers
{
    public class PlanKPIDetail_KPIApiController : ApiController
    {
        [Authorize]
        [Route("")]
        public int GetMaxOrderNumberKPI(Guid planKPIDetailId)
        {
            int result = 0;
            List<PlanKPIDetail_KPI> list = new List<PlanKPIDetail_KPI>();
            SessionManager.DoWork(session =>
            {
                list = session.Query<PlanKPIDetail_KPI>().Where(p => p.PlanKPIDetail.Id == planKPIDetailId).ToList();
                if (list.Count > 0)
                    result = session.Query<PlanKPIDetail_KPI>().Where(p => p.PlanKPIDetail.Id == planKPIDetailId).Max(p => p.OrderNumber);
            });
            return result;
        }
