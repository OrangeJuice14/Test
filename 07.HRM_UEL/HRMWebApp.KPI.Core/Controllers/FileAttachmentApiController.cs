...
namespace HRMWebApp.KPI.Core.Controllers
{
    public class FileAttachmentApiController : ApiController
    {
        ...
        [Authorize]
        [Route("")]
        public IEnumerable<FileAttachmentDTO> GetListByActivityDetail(Guid Id)
        {
            var result = new List<FileAttachmentDTO>();
            SessionManager.DoWork(session =>
            {
                result = session.Query<FileAttachment>().Where(d => d.ProfessorOtherActivity.Id == Id).ToList().Map<FileAttachmentDTO>();
            });
            return result;
        }
