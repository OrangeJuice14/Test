...
namespace HRMWebApp.KPI.Core.Controllers
{
    public class DepartmentApiController : ApiController
    {
        ...
        public IEnumerable<DepartmentDTO> GetList()
        {
            ...
            {
                Guid truong = new Guid("B04BFE58-29B6-4339-BA93-C4ACA2D5CA6A"); //Trường Đại học Ngân hàng
        ...
        [Authorize]
        [Route("")]
        public IEnumerable<DepartmentDTO> GetListAll()
        {
            var result = new List<DepartmentDTO>();
            SessionManager.DoWork(session =>
            {
                result = session.Query<Department>().Where(d => d.GCRecord == null && d.IsDisable == false).OrderBy(d => d.Name).ToList().Map<DepartmentDTO>();
            });
            return result;
        }
        ...
        public IEnumerable<DepartmentHierarchyDTO> GetAdminDepartmentListHierarchy(Guid adminId)
        {
            ...
            SessionManager.DoWork(session =>
            {
                ...
                Guid truong = new Guid("B04BFE58-29B6-4339-BA93-C4ACA2D5CA6A");
                List<Department> deptList = new List<Department>();
                deptList = session.Query<Department>().Where(d => d.ParentDepartment.Id == truong && d.GCRecord == null && d.IsDisable == false).OrderBy(d => d.Name).ToList();
