    ...
    public class StaffApiController : ApiController
    {
        ...
        public IEnumerable<StaffDTO> GetListAdminLeader()
        {
            ...
            SessionManager.DoWork(session =>
            {
                ...
                templist = session.Query<Staff>().Where(s => s.StaffStatus.NoLongerWork == 0 && s.StaffProfile.GCRecord == null && s.StaffInfo.Position.Name.Contains("hiệu")).ToList();
                foreach (Staff s in templist)
                {
                    StaffDTO sd = new StaffDTO();
                    sd = ParseStaff(s);
        ...
        public StaffDTO GetStaffDTOById(Guid id)
        {
            var result = new StaffDTO();
            SessionManager.DoWork(session =>
            {
                ...
                if (staff != null)
                {
                    result.DepartmentId = staff.Department != null ? staff.Department.Id : Guid.Empty;
                    result.DepartmentName = staff.Department != null ? staff.Department.Name : "";
                    result.Name = staff.StaffProfile != null ? staff.StaffProfile.Name : "";
                    result.PositionName = staff.StaffInfo != null && staff.StaffInfo.Position != null ? staff.StaffInfo.Position.Name : "";
                }
            });
            return result;
        }
